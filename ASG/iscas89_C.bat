#ASG.exe -set ./input/script/s27_C.set

#ASG.exe -set ./input/script/s208_C.set

#ASG.exe -set ./input/script/s298_C.set

#ASG.exe -set ./input/script/s344_C.set

#ASG.exe -set ./input/script/s349_C.set

#ASG.exe -set ./input/script/s382_C.set

#ASG.exe -set ./input/script/s386_C.set

#ASG.exe -set ./input/script/s400_C.set

#ASG.exe -set ./input/script/s420_C.set

#ASG.exe -set ./input/script/s444_C.set

#ASG.exe -set ./input/script/s510_C.set

#ASG.exe -set ./input/script/s526_C.set

#ASG.exe -set ./input/script/s641_C.set

ASG.exe -set ./input/script/s713_C.set

ASG.exe -set ./input/script/s820_C.set

ASG.exe -set ./input/script/s832_C.set

ASG.exe -set ./input/script/s838_C.set

ASG.exe -set ./input/script/s953_C.set

ASG.exe -set ./input/script/s1196_C.set

ASG.exe -set ./input/script/s1238_C.set

ASG.exe -set ./input/script/s1423_C.set

ASG.exe -set ./input/script/s1488_C.set

ASG.exe -set ./input/script/s1494_C.set

more