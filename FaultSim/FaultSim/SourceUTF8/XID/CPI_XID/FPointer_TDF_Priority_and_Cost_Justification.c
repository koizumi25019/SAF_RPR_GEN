//------------------------------------------------------------------------
//File name : FPointer_TDF_Priority_and_Cost_Justification.c
//Date : 2013/12/5
//Designer : H.Yamazaki
//Ver : 0.01
//------------------------------------------------------------------------
...
//限定正当化
void	TDF_XID_pri_cost_jus_and_nand		(S_NLIST*, int, unsigned int, unsigned int, unsigned int);
void	TDF_XID_pri_cost_jus_or_nor			(S_NLIST*, int, unsigned int, unsigned int, unsigned int);
...
//------------------------------------------------------------------------
//----------------------------------------------
//  関数名 : FPointer_TDF_Priority_and_Cost_Justification
//  機  能 : 【関数ポインタ作成】TDF用の正当化(限定含意・限定正当化)の関数ポインタ作成
//  戻り値 : なし
//  引  数 : なし
// ※優先パスと可制御性を優先探索
//----------------------------------------------
void	FPointer_TDF_Priority_and_Cost_Justification(){
	...
	//======================================================================================
	//優先パス&可制御性優先の限定正当化
	sfunc_prijus[AND]	= TDF_XID_pri_cost_jus_and_nand;	//ANDとNANDは同じ処理
	sfunc_prijus[NAND]	= TDF_XID_pri_cost_jus_and_nand;	//ANDとNANDは同じ処理
	sfunc_prijus[OR]	= TDF_XID_pri_cost_jus_or_nor;		//ORとNORは同じ処理
	sfunc_prijus[NOR]	= TDF_XID_pri_cost_jus_or_nor;		//ORとNORは同じ処理
}
...
//★限定正当化★
//------------------------------------------------------------------------------------
//  関数名 : TDF_XID_pri_cost_jus_and_nand
//  機  能 : 【AND・NAND】2値・3値限定正当化
//  戻り値 : なし
//  引  数 : t_net(対象信号線), tp_id(テストパターンの何番目か), xid_flag(現在のケアビット固定フラグ値), jus_flag(限定正当化フラグ), priority(優先フラグ)
// ※優先パスと可制御性を優先探索
//------------------------------------------------------------------------------------
void	TDF_XID_pri_cost_jus_and_nand(S_NLIST *t_net, int tp_id, unsigned int xid_flag, unsigned int jus_flag, unsigned int priority){

	int				i;
	int				in_id;						//入力信号線の最小可制御性の入力ID
			...
			for(i=0; i<t_net->n_in; i++){
				...
				if(Get_NBit_Xbuf(t_net->in[i]->nval_t1, tp_id) == 0){
					...
					//★xid_nvalが『X』の場合
						...
						//最小可制御性なら上書き
						if(in_cost > t_net->in[i]->cost[C_0][0]){
							in_cost = t_net->in[i]->cost[C_0][0];	//一番0可制御性が低い信号線を更新
							in_id = i;								//入力IDを保存
						}
					}
...
//------------------------------------------------------------------------------------
//  関数名 : TDF_XID_pri_cost_jus_or_nor
//  機  能 : 【OR・NOR】2値・3値優先パス限定正当化
//  戻り値 : なし
//  引  数 : t_net(対象信号線), tp_id(テストパターンの何番目か), xid_flag(現在のケアビット固定フラグ値), jus_flag(限定正当化フラグ), priority(優先フラグ)
// ※優先パスと可制御性を優先探索
//------------------------------------------------------------------------------------
void	TDF_XID_pri_cost_jus_or_nor(S_NLIST *t_net, int tp_id, unsigned int xid_flag, unsigned int jus_flag, unsigned int priority){
			...
			for(i=0; i<t_net->n_in; i++){
				...
				if(Get_NBit_Xbuf(t_net->in[i]->nval_t1, tp_id) == 1){
					...
					//★xid_nvalが『X』の場合
						...
						//最小可制御性なら上書き
						if(in_cost > t_net->in[i]->cost[C_1][0]){
							in_cost = t_net->in[i]->cost[C_1][0];	//一番1可制御性が低い信号線を更新
							in_id = i;								//入力IDを保存
						}
					}
				}
