//------------------------------------------------------------------------
//File name : TDF_priority_2v_xfilling
//Date : 2013/11/22
//Designer : H.Yamazaki
//Ver : 0.01
//------------------------------------------------------------------------
...
//----------------------------------------------
unsigned int	TDF_priority_2v_xfilling(int tp_id, SXID_LIST **t_fault, int n_fault, unsigned int ed_flag, unsigned int xid_flag, unsigned int *jus_flag, unsigned int priority){
	...
	*/
	...
	for(i=0; i<n_fault; i++){
		printf("%d番目 対象故障：%s  ",i, t_fault[i]->net->name);
			...
			if(sort_propa_po[j]->n_det > 0){
				...
				if(Get_NBit_INT(sort_propa_po[j]->det_flag, i) == 1){
...
#ifdef PATH_DEBUG
					printf("\n\n【X抽出開始】\n");
					printf("★%d番目の故障：%s  ",i, t_fault[i]->net->name);
					if(t_fault[i]->fault_type == TDF_STR){
						printf("STR");
					}
					else if(t_fault[i]->fault_type == TDF_STF){
					...
					if(t_fault[i]->fault_type == TDF_STR){
						...
						//1時刻目対象信号線がドントケアの場合
						if(Get_NBit(t_fault[i]->net->xid_nval_t1, tp_id) == 3){
#ifdef TIME1_DEBUG
							printf("初期値0固定(時刻1):%s\n", t_fault[i]->net->name);
#endif
							//--------------------
							//1時刻目0固定
							//--------------------
							Set_NBit_Zero(t_fault[i]->net->xid_nval_t1, tp_id);
							
							//--------------------
							//後方含意判定
							//--------------------
							//PIの場合
							if(t_fault[i]->net->type == IN){
								printf("\n//----------------------------------\n");
							...
							//PPIの場合
							else if(t_fault[i]->net->type == DFF){
								//テストパターンに対するケアビット情報
								tp_bit[tp_id].n_cbit++;						//対象信号線(PPI)のテストパターン[tp_id]のケアビット数をインクリメント
								tp_bit[tp_id].n_0bit++;						//対象信号線(PPI)のテストパターン[tp_id]の0数をインクリメント

								//PPIに対するケアビット情報
								ppi_bit[t_fault[i]->net->ppi_id].n_cbit++;	//対象信号線(PPI)のケアビット数をインクリメント
								ppi_bit[t_fault[i]->net->ppi_id].n_0bit++;	//対象信号線(PPI)の0数をインクリメント
							}
							//その他→後方含意
							else{
								TDF_0_Backward_Implication(t_fault[i]->net, tp_id, 1, *jus_flag);
							}						

							//--------------------
							//PPOの場合：2時刻目前方含意
							//--------------------
							if(t_fault[i]->net->ppo_flag == YES){
								for(k=0; k<t_fault[i]->net->n_out; k++){
									if(Get_NBit(t_fault[i]->net->out[k]->xid_nval_t2, tp_id) == 3){
										TDF_0_Forward_Implication(t_fault[i]->net->out[k], tp_id, 2);
									}
								}
							}
							...
							//--------------------
								...
								for(k=0; k<t_fault[i]->net->n_out; k++){
									if(Get_NBit(t_fault[i]->net->out[k]->xid_nval_t1, tp_id) == 3){
										TDF_0_Forward_Implication(t_fault[i]->net->out[k], tp_id, 1);
									}
								}
							}
						}
					...
					//--------------------------------------------------
						...
						if(Get_NBit(t_fault[i]->net->xid_nval_t1, tp_id) == 3){
							...
							//1時刻目1固定
							//--------------------
							Set_NBit_One(t_fault[i]->net->xid_nval_t1, tp_id);
							...
							//PPIの場合
								...
								//PPIに対するケアビット情報
								ppi_bit[t_fault[i]->net->ppi_id].n_cbit++;	//対象信号線(PPI)のケアビット数をインクリメント
								ppi_bit[t_fault[i]->net->ppi_id].n_1bit++;	//対象信号線(PPI)の1数をインクリメント
							}
							//その他→後方含意
							else{
								TDF_1_Backward_Implication(t_fault[i]->net, tp_id, 1, *jus_flag);
							}
							...
							if(t_fault[i]->net->ppo_flag == YES){
								for(k=0; k<t_fault[i]->net->n_out; k++){
									if(Get_NBit(t_fault[i]->net->out[k]->xid_nval_t2, tp_id) == 3){
										TDF_1_Forward_Implication(t_fault[i]->net->out[k], tp_id, 2);
									}
								}
							}
							...
							//--------------------
								...
								for(k=0; k<t_fault[i]->net->n_out; k++){
									if(Get_NBit(t_fault[i]->net->out[k]->xid_nval_t1, tp_id) == 3){
										TDF_1_Forward_Implication(t_fault[i]->net->out[k], tp_id, 1);
									}
								}
							}
						}
					...
					printf("\n\n【故障伝搬経路決定】\n");
					printf("故障伝搬PPO: %s\n", ppo[sort_propa_po[j]->po_id]->name);
					printf("対象故障：%s  TDF%d ed_flag:%d xid_flag:%d jus_flag:%d\n\n",t_fault[i]->net->name, t_fault[i]->fault_type, ed_flag, xid_flag, *jus_flag);
#endif
					//PPOをプッシュ
					...
					while(1){
						...
						//-------------------------------------------
						// ★優先パス経路選択 : temp_netの伝搬経路計算
						//-------------------------------------------
						(*sfunc_pripath[temp_net->type])(temp_net, tp_id, i, ed_flag, xid_flag, *jus_flag, priority);
...
#endif
							//★優先パス正当化(関数ポインタ)
							(*sfunc_prijus[temp_net->type])(temp_net, tp_id, xid_flag, *jus_flag, priority);
						}
					}
