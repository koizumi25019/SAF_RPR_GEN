...
unsigned int	SAF_2v_missed_xfilling				(int, XID_LIST**, int, unsigned int, unsigned int);
unsigned int	SAF_3v_missed_xfilling				(int, XID_LIST**, int, unsigned int, unsigned int);
...
//----------------------------------------------
//  関数名 : SAF_Missed_XID
//  機  能 : 【見逃し故障】縮退故障ドントケア抽出(正常値と故障値の両方を確認)
//  戻り値 : なし
//  引  数 : t_fault(X抽出する必須故障リスト)
//----------------------------------------------
void	SAF_Missed_XID(XID_LIST	**t_fault){
	...
	// 関数ポインタ再セッティング
	//===================================================================
	FPointer_SAF_Missed_Justification();
	FPointer_SAF_Missed_Fault_Path();

	//===================================================================
	// 見逃し故障 X抽出
	//===================================================================
	fprintf(stderr,"\n\n X-Identification for Missed Fault (Missed-XID)\n");
	fprintf(stderr,"|----+----|----+----|----+----|----+----|----+----|\n");
				...
				if( Get_NBit_INT(fdic_sa0[i], j)==YES ){	//対象TPで検出するか？
					...
					if(count == 31){
						...
						if(n_before_x == 0){
							...
							xid_flag = SAF_2v_missed_xfilling(i, t_fault, count, ed_flag, xid_flag);//SPPFPからの2値XID
						}
						//【3値FSIM】&【X抽出】
						else{
							SAF_3v_SPPFP(i, t_fault, count, ed_flag);							//SPPFP故障SIM
							xid_flag = SAF_3v_missed_xfilling(i, t_fault, count, ed_flag, xid_flag);//SPPFPからの3値XID
						}
