...
//----------------------------------------------
unsigned int	SAF_2v_EX_xfilling(int tp_id, XID_LIST **t_fault, int n_fault, unsigned int ed_flag, unsigned int xid_flag){
		...
		for(j=0; j<n_po; j++){
			...
			if(sort_propa_po[j]->n_det > 0){
				...
				if(Get_NBit_INT(sort_propa_po[j]->det_flag, i) == 1){
					...
					//======================================================================================
					// 【拡張含意操作】POから到達可能な全PIまでを全てケアビット固定
					//======================================================================================
					while(1){
						...
						for(k=0; k<temp_net->n_in; k++){
							...
							if(temp_net->in[k]->xid_flag != xid_flag){
								...
								//lev_tempに対象信号線をプッシュ
								lev_temp[temp_net->in[k]->level].net[lev_temp[temp_net->in[k]->level].n_net] = temp_net->in[k];
		
								//lev_tempの保持信号線数をインクリメント
								lev_temp[temp_net->in[k]->level].n_net++;
								...
								if(Get_NBit(temp_net->in[k]->xid_nval, tp_id) == 3){
									...
									//対象信号線の正常値を取得
									nval = Get_NBit_Xbuf(temp_net->in[k]->nval, tp_id);
