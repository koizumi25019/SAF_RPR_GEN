...
//------------------------------------------------------------------------
//------------------------------------------------------------------------------------
//  関数名 : SAF_2v_CPT0_Ndrop
//  機  能 : 2値縮退故障クリティカルパストレーシング
//  戻り値 : なし
//  引  数 : ffr_id(FFR番号), tp_id(テストパターンの何番目か), t_net(CPTを始める信号線[=FFRの先頭])
//------------------------------------------------------------------------------------
void	SAF_2v_CPT0_Ndrop(int ffr_id, unsigned int tp_id, NLIST *t_net){
	...
	switch(t_net->type){
		...
		case BUF:
			...
			//さらに入力信号線をCPT
			SAF_2v_CPT0_Ndrop(ffr_id, tp_id, t_net->in[0]);
		...
		case INV:
			...
			//さらに入力信号線をCPT
			SAF_2v_CPT1_Ndrop(ffr_id, tp_id, t_net->in[0]);
		...
		case AND:
			...
			if(n_contorolling==1){
				...
				//入力信号線のさらに入力信号線をCPT
				SAF_2v_CPT0_Ndrop(ffr_id, tp_id, t_net->in[in]);
			}
		...
		case NAND:
			...
			for(i=0; i<t_net->n_in; i++){
				...
				//さらに入力信号線をCPT
				SAF_2v_CPT1_Ndrop(ffr_id, tp_id, t_net->in[i]);
			}
		...
		case OR:
			...
			for(i=0; i<t_net->n_in; i++){
				...
				//さらに入力信号線をCPT
				SAF_2v_CPT0_Ndrop(ffr_id, tp_id, t_net->in[i]);
			}
		...
		case NOR:
			...
			if(n_contorolling==1){
				...
				//入力信号線のさらに入力信号線をCPT
				SAF_2v_CPT1_Ndrop(ffr_id, tp_id, t_net->in[in]);
			}
...
//******************************************************************************
//------------------------------------------------------------------------------------
//  関数名 : SAF_2v_CPT1_Ndrop
//  機  能 : 2値縮退故障クリティカルパストレーシング
//  戻り値 : なし
//  引  数 : ffr_id(FFR番号), tp_id(テストパターンの何番目か), t_net(CPTを始める信号線[=FFRの先頭])
//------------------------------------------------------------------------------------
void	SAF_2v_CPT1_Ndrop(int ffr_id, unsigned int tp_id, NLIST *t_net){
