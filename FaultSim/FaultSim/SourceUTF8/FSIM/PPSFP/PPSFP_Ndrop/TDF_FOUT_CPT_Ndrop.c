//------------------------------------------------------------------------
//File name : TDF_FOUT_CPT_Ndrop.c
//Date : 2013/2/1
//Designer : H.Yamazaki
//Ver : 0.01
//------------------------------------------------------------------------
...
//----------------------------------------------
void	TDF_FOUT_2v_CPT_Ndrop	(int ffr_id, S_NLIST* stem_net, int ui_num, unsigned int* det_tp_list, S_NLIST* t_ppo){
				...
				if( (fault_tp & MASKbit[i]) != 0){					
					...
					if(x_buff != 0){	//i番目に1が立ってる
						...
						*det_tp_list |= MASKbit[i];						//det_tp_listのi番目に『1』を立てる
						...
						if(sffr[ffr_id].FoutStem->det_str<opt.n_drop && sffr[ffr_id].FoutStem->test_str==YES){
						...
						//正常値1のCPT開始
						TDF_2v_CPT1_Ndrop(ffr_id, tp_id, stem_net);
					}
					...
					//==========================================
						...
						*det_tp_list |= MASKbit[i];					//det_tp_listのi番目に『1』を立てる
						...
						//検出回数がopt.n_drop回以下なら検出回数等を保存			
						if(sffr[ffr_id].FoutStem->det_stf<opt.n_drop && sffr[ffr_id].FoutStem->test_stf==YES){
						...
						//正常値0のCPT開始
						TDF_2v_CPT0_Ndrop(ffr_id, tp_id, stem_net);
					}
				}
			}
		}
...
//----------------------------------------------
//  関数名 : TDF_FOUT_3v_CPT_Ndrop
//  機  能 : POまで故障伝搬したTPが何番目か判定し，CPT関数実行
//  戻り値 : なし
//  引  数 : ffr_id(ステムのFFR番号), stem_net(ステム信号線), ui_num(unsigned int番目), det_tp_list(PPOまで故障伝搬したTPリスト【0:未伝搬 1:伝搬(=故障検出)】), t_ppo(故障伝搬したPO)
//----------------------------------------------
void	TDF_FOUT_3v_CPT_Ndrop	(int ffr_id, S_NLIST* stem_net, int ui_num, unsigned int* det_tp_list, S_NLIST* t_ppo){
					...
					if((x_buff&p_buff)==0){
						...
						if(x_buff != 0){	//i番目に1が立ってる
							...
							*det_tp_list |= MASKbit[i];			//det_tp_listのi番目に『1』を立てる
							...
							//正常値1のCPT開始
							TDF_3v_CPT1_Ndrop(ffr_id, tp_id, stem_net);
						}
						...
						//==========================================
							...
							*det_tp_list |= MASKbit[i];		//det_tp_listのi番目に『1』を立てる
							...
							//正常値0のCPT開始
							TDF_3v_CPT0_Ndrop(ffr_id, tp_id, stem_net);
						}
					}
				}
			}
