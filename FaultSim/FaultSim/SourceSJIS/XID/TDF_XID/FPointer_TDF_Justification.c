...
//限定正当化
void	TDF_XID_jus_and_nand				(S_NLIST*, int, unsigned int, unsigned int);
void	TDF_XID_jus_or_nor					(S_NLIST*, int, unsigned int, unsigned int);
...
//------------------------------------------------------------------------
//----------------------------------------------
//  関数名 : FPointer_TDF_Justification
...
void	FPointer_TDF_Justification(){
	...
	//======================================================================================
	//限定正当化
	sfunc_jus[AND]	= TDF_XID_jus_and_nand;		//ANDとNANDは同じ処理
	sfunc_jus[NAND]	= TDF_XID_jus_and_nand;		//ANDとNANDは同じ処理
	sfunc_jus[OR]	= TDF_XID_jus_or_nor;		//ORとNORは同じ処理
	sfunc_jus[NOR]	= TDF_XID_jus_or_nor;		//ORとNORは同じ処理
}
...
//★限定正当化★
//------------------------------------------------------------------------------------
//  関数名 : TDF_XID_jus_and_nand
//  機  能 : 【AND・NAND】2値・3値限定正当化
//  戻り値 : なし
//  引  数 : t_net(対象信号線), tp_id(テストパターンの何番目か), xid_flag(現在のケアビット固定フラグ値), jus_flag(限定正当化フラグ)
//------------------------------------------------------------------------------------
void	TDF_XID_jus_and_nand(S_NLIST *t_net, int tp_id, unsigned int xid_flag, unsigned int jus_flag){
...
//------------------------------------------------------------------------------------
//  関数名 : TDF_XID_jus_or_nor
//  機  能 : 【OR・NOR】2値・3値限定正当化
//  戻り値 : なし
//  引  数 : t_net(対象信号線), tp_id(テストパターンの何番目か), xid_flag(現在のケアビット固定フラグ値), jus_flag(限定正当化フラグ)
//------------------------------------------------------------------------------------
void	TDF_XID_jus_or_nor(S_NLIST *t_net, int tp_id, unsigned int xid_flag, unsigned int jus_flag){
