...
//------------------------------------------------------------------------
//----------------------------------------------
//  関数名 : TDF_Essential_XID
//  機  能 : 【必須故障：限定含意・限定正当化】遷移故障ドントケア抽出
//  戻り値 : なし
//  引  数 : t_fault(X抽出対象故障リストの空の箱)
//----------------------------------------------
void	TDF_Essential_XID(SXID_LIST	**t_fault){
	...
	int				count=0;		//t_fault内に何個必須故障を入れたかのカウンタ
		...
		for(i=0; i<n_tp; i++){
			...
			for(j=0; j<n_snet; j++){
				// 立上り遷移故障(0→1/0)
				if(s_nl[j].test_str==YES && s_nl[j].det_str==1 && s_nl[j].xid_det_str==NO){	//必須故障かつ未検出？
					if( Get_NBit_INT(fdic_str[i], j)==YES ){	//対象TPで検出するか？
						printf("STR %s\n", s_nl[j].name);
					}
				}						
				// 立下り遷移故障(1→0/1)
				if(s_nl[j].test_stf==YES && s_nl[j].det_stf==1 && s_nl[j].xid_det_stf==NO){	//必須故障かつ未検出？
