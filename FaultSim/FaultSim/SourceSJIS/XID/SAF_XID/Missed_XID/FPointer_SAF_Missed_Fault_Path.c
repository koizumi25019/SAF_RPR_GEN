//------------------------------------------------------------------------
//File name : FPointer_SAF_Missed_Fault_Path.c
//Date : 2013/1/27
//Designer : H.Yamazaki
//Ver : 0.01
//------------------------------------------------------------------------
...
void		SAF_0_Missed_Backward_Implication	(NLIST*, int, int, unsigned int);
void		SAF_1_Missed_Backward_Implication	(NLIST*, int, int, unsigned int);


//伝搬経路決定
void		SAF_XID_missed_fpath_single_gate	(NLIST*, int, int, unsigned int, unsigned int);
void		SAF_XID_missed_fpath_and			(NLIST*, int, int, unsigned int, unsigned int);
void		SAF_XID_missed_fpath_nand			(NLIST*, int, int, unsigned int, unsigned int);
void		SAF_XID_missed_fpath_or				(NLIST*, int, int, unsigned int, unsigned int);
void		SAF_XID_missed_fpath_nor			(NLIST*, int, int, unsigned int, unsigned int);
void		SAF_XID_missed_fpath_exor			(NLIST*, int, int, unsigned int, unsigned int);
...
//------------------------------------------------------------------------
//----------------------------------------------
//  関数名 : FPointer_SAF_Missed_Fault_Path
//  機  能 : 【関数ポインタ作成】SAF用の故障伝搬経路決定の関数ポインタ作成
//  戻り値 : なし
//  引  数 : なし
//----------------------------------------------
void	FPointer_SAF_Missed_Fault_Path(){
		
	//======================================================================================
	// 関数ポインタ作成
	//======================================================================================
	//故障伝搬経路
	func_path[FOUT]	 = SAF_XID_missed_fpath_single_gate;
	func_path[BUF]	 = SAF_XID_missed_fpath_single_gate;
	func_path[INV]	 = SAF_XID_missed_fpath_single_gate;
	func_path[AND]	 = SAF_XID_missed_fpath_and;
	func_path[NAND]	 = SAF_XID_missed_fpath_nand;
	func_path[OR]	 = SAF_XID_missed_fpath_or;
	func_path[NOR]	 = SAF_XID_missed_fpath_nor;
	func_path[EXOR]	 = SAF_XID_missed_fpath_exor;
	func_path[EXNOR] = SAF_XID_missed_fpath_exor;
}
...
//★【2値・3値共通】故障伝搬経路★
//------------------------------------------------------------------------------------
//  関数名 : SAF_XID_missed_fpath_single_gate
...
//------------------------------------------------------------------------------------
void	SAF_XID_missed_fpath_single_gate(NLIST *t_net, int tp_id, int fault_id, unsigned int ed_flag, unsigned int xid_flag){
...
//------------------------------------------------------------------------------------
//  関数名 : SAF_XID_missed_fpath_and
//  機  能 : 【AND】故障伝搬経路決定
//  戻り値 : なし
//  引  数 : t_net(対象信号線), tp_id(テストパターンの何番目か), fault_id(32個中何番目の故障なのか), ed_flag(FSIMイベントフラグ値), xid_flag(X抽出(ケアビット固定)フラグ値)
//------------------------------------------------------------------------------------
void	SAF_XID_missed_fpath_and(NLIST *t_net, int tp_id, int fault_id, unsigned int ed_flag, unsigned int xid_flag){
...
#endif					
						//1からの見逃し故障用の後方含意
						SAF_1_Missed_Backward_Implication(t_net->in[i], tp_id, fault_id, ed_flag);
					}
					//★入力信号線が固定済みでも見逃し故障用後方含意
					else{
#ifdef PATH_DEBUG
						printf("【見逃し固定】%s : 1固定済\n", t_net->in[i]->name);
#endif	
...
//------------------------------------------------------------------------------------
//  関数名 : SAF_XID_missed_fpath_nand
//  機  能 : 【NAND】故障伝搬経路決定
//  戻り値 : なし
//  引  数 : t_net(対象信号線), tp_id(テストパターンの何番目か), fault_id(32個中何番目の故障なのか), ed_flag(FSIMイベントフラグ値), xid_flag(X抽出(ケアビット固定)フラグ値)
//------------------------------------------------------------------------------------
void	SAF_XID_missed_fpath_nand(NLIST *t_net, int tp_id, int fault_id, unsigned int ed_flag, unsigned int xid_flag){
...
//------------------------------------------------------------------------------------
//  関数名 : SAF_XID_missed_fpath_or
//  機  能 : 【OR】故障伝搬経路決定
//  戻り値 : なし
//  引  数 : t_net(対象信号線), tp_id(テストパターンの何番目か), fault_id(32個中何番目の故障なのか), ed_flag(FSIMイベントフラグ値), xid_flag(X抽出(ケアビット固定)フラグ値)
//------------------------------------------------------------------------------------
void	SAF_XID_missed_fpath_or(NLIST *t_net, int tp_id, int fault_id, unsigned int ed_flag, unsigned int xid_flag){
...
#endif					
						//0からの見逃し故障用の後方含意
						SAF_0_Missed_Backward_Implication(t_net->in[i], tp_id, fault_id, ed_flag);
					}
					//★入力信号線が固定済みでも見逃し故障用後方含意
					else{
#ifdef PATH_DEBUG
...
//------------------------------------------------------------------------------------
//  関数名 : SAF_XID_missed_fpath_nor
//  機  能 : 【NOR】故障伝搬経路決定
//  戻り値 : なし
//  引  数 : t_net(対象信号線), tp_id(テストパターンの何番目か), fault_id(32個中何番目の故障なのか), ed_flag(FSIMイベントフラグ値), xid_flag(X抽出(ケアビット固定)フラグ値)
//------------------------------------------------------------------------------------
void	SAF_XID_missed_fpath_nor(NLIST *t_net, int tp_id, int fault_id, unsigned int ed_flag, unsigned int xid_flag){
...
//------------------------------------------------------------------------------------
//  関数名 : SAF_XID_missed_fpath_exor
//  機  能 : 【EXOR・EXNOR】故障伝搬経路決定
//  戻り値 : なし
//  引  数 : t_net(対象信号線), tp_id(テストパターンの何番目か), fault_id(32個中何番目の故障なのか), ed_flag(FSIMイベントフラグ値), xid_flag(X抽出(ケアビット固定)フラグ値)
//※2入力のみ対応
//------------------------------------------------------------------------------------
void	SAF_XID_missed_fpath_exor(NLIST *t_net, int tp_id, int fault_id, unsigned int ed_flag, unsigned int xid_flag){
