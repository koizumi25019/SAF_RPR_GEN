...
//	・故障伝搬経路	：
//	・後方含意		：
//	・前方含意		：変更な(むしろやらなくても良いかも)
//	・正当化		：
//----------------------------------------------
unsigned int	SAF_2v_missed_xfilling(int tp_id, XID_LIST **t_fault, int n_fault, unsigned int ed_flag, unsigned int xid_flag){
			...
			if(sort_propa_po[j]->n_det > 0){
				...
				if(Get_NBit_INT(sort_propa_po[j]->det_flag, i) == 1){
					...
					switch(temp_net->type){
						...
						case BUF:
							...
							if(Get_NBit_Xbuf(temp_net->xid_nval, tp_id) == 0){
								SAF_0_Missed_Backward_Implication(temp_net, tp_id, i, ed_flag);
							}
							//出力信号線=1
							else{
								SAF_1_Missed_Backward_Implication(temp_net, tp_id, i, ed_flag);
							}
					...
					while((temp_net = (NLIST *)dequeue(nl_queue)) != (NLIST *)NULL){	//空になるまでデキュー
						...
						//-------------------------------------------
						// 【関数ポインタ使用】temp_netに対して見逃し故障用の限定正当化
						//-------------------------------------------
						if(temp_net->n_in != 0){
#ifdef ECX_DEBUG
							printf("限定正当化: %s\n", temp_net->name);
#endif
							//見逃し故障用の正当化(関数ポインタ)
							(*func_miss_jus[temp_net->type])(temp_net, tp_id, xid_flag, ed_flag, i);
						}
					}
