...
unsigned int	SAF_2v_diff_path_xfilling				(int, XID_LIST**, int, unsigned int, unsigned int);
unsigned int	SAF_3v_diff_path_xfilling				(int, XID_LIST**, int, unsigned int, unsigned int);
...
//------------------------------------------------------------------------
//----------------------------------------------
//  関数名 : SAF_diff_path_XID
//  機  能 : 【縮退故障】通常とは異なるパスでX抽出
//  戻り値 : なし
//  引  数 : t_fault(X抽出対象故障リストの空の箱)
//----------------------------------------------
void	SAF_diff_path_XID(XID_LIST	**t_fault){
	...
	// 乱数初期シード初期化
	//===================================================================
	init_genrand(997);			//time関数でも良いけど，実行毎に結果が変わってしまうので固定値にしておく
	
			
	//===================================================================
	// 関数ポインタ再セット
	//===================================================================
	FPointer_SAF_Diff_Fault_Path();
	FPointer_SAF_diff_path_Justification();

	//===================================================================
	// Different Path X抽出
	//===================================================================
	fprintf(stderr,"\n\n Different Path X-Identification\n");
	fprintf(stderr,"|----+----|----+----|----+----|----+----|----+----|\n");
	...
	for(i=0; i<n_tp; i++){
		...
		for(j=0; j<n_net; j++){
			...
			// ■0縮退故障の対象故障判定
			//====================================================================================
			if(nl[j].test_sf0==YES && nl[j].det_sf0>1 && nl[j].xid_det_sf0==NO && nl[j].n_sa0_diff_xid<opt.diff_path_n_xid){	//対象故障かつ1回以上検出かつ未検出？
				...
				if( Get_NBit_INT(fdic_sa0[i], j)==YES ){	//対象TPで検出するか？
					...
					if(count == 31){
						...
						if(n_before_x == 0){
							...
							xid_flag = SAF_2v_diff_path_xfilling(i, t_fault, count, ed_flag, xid_flag);	//SPPFPからの2値XID
						}
						//【3値FSIM】&【X抽出】
						else{
							SAF_3v_SPPFP(i, t_fault, count, ed_flag);									//SPPFP故障SIM
							xid_flag = SAF_3v_diff_path_xfilling(i, t_fault, count, ed_flag, xid_flag);	//SPPFPからの3値XID
						}
			...
			// ■1縮退故障の対象故障判定
			//====================================================================================
			if(nl[j].test_sf1==YES && nl[j].det_sf1>1 && nl[j].xid_det_sf1==NO && nl[j].n_sa1_diff_xid<opt.diff_path_n_xid){	//対象故障かつ1回以上検出かつ未検出？
