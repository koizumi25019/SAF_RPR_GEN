//------------------------------------------------------------------------
//File name : FPointer_TDF_Priority_Fault_Path.c
//Date : 2013/12/5
//Designer : H.Yamazaki
//Ver : 0.01
//------------------------------------------------------------------------
...
//伝搬経路決定
void		TDF_XID_pri_cost_fpath_single_gate	(S_NLIST*, int, int, unsigned int, unsigned int, unsigned int, unsigned int);
void		TDF_XID_pri_cost_fpath_and			(S_NLIST*, int, int, unsigned int, unsigned int, unsigned int, unsigned int);
void		TDF_XID_pri_cost_fpath_nand			(S_NLIST*, int, int, unsigned int, unsigned int, unsigned int, unsigned int);
void		TDF_XID_pri_cost_fpath_or			(S_NLIST*, int, int, unsigned int, unsigned int, unsigned int, unsigned int);
void		TDF_XID_pri_cost_fpath_nor			(S_NLIST*, int, int, unsigned int, unsigned int, unsigned int, unsigned int);
void		TDF_XID_pri_cost_fpath_exor			(S_NLIST*, int, int, unsigned int, unsigned int, unsigned int, unsigned int);
...
//------------------------------------------------------------------------
//----------------------------------------------
//  関数名 : FPointer_TDF_Priority_Cost_Fault_Path
...
void	FPointer_TDF_Priority_Cost_Fault_Path(){
	...
	//======================================================================================
	//故障伝搬経路
	sfunc_pripath[FOUT]		= TDF_XID_pri_cost_fpath_single_gate;
	sfunc_pripath[BUF]		= TDF_XID_pri_cost_fpath_single_gate;
	sfunc_pripath[INV]		= TDF_XID_pri_cost_fpath_single_gate;
	sfunc_pripath[AND]		= TDF_XID_pri_cost_fpath_and;
	sfunc_pripath[NAND]		= TDF_XID_pri_cost_fpath_nand;
	sfunc_pripath[OR]		= TDF_XID_pri_cost_fpath_or;
	sfunc_pripath[NOR]		= TDF_XID_pri_cost_fpath_nor;
	sfunc_pripath[EXOR]		= TDF_XID_pri_cost_fpath_exor;
	sfunc_pripath[EXNOR]	= TDF_XID_pri_cost_fpath_exor;
}
...
//★【2値・3値共通】故障伝搬経路★
//------------------------------------------------------------------------------------
//  関数名 : TDF_XID_pri_cost_fpath_single_gate
//  機  能 : 【FOUT・BUF・INV】故障伝搬経路決定
//  戻り値 : なし
//  引  数 : t_net(対象信号線), tp_id(テストパターンの何番目か), fault_id(32個中何番目の故障なのか), ed_flag(FSIMイベントフラグ値), xid_flag(X抽出(ケアビット固定)フラグ値), priority(優先フラグ)
//------------------------------------------------------------------------------------
void	TDF_XID_pri_cost_fpath_single_gate(S_NLIST *t_net, int tp_id, int fault_id, unsigned int ed_flag, unsigned int xid_flag, unsigned int jus_flag, unsigned int priority){
...
//------------------------------------------------------------------------------------
//  関数名 : TDF_XID_pri_cost_fpath_and
//  機  能 : 【AND】故障伝搬経路決定
//  戻り値 : なし
//  引  数 : t_net(対象信号線), tp_id(テストパターンの何番目か), fault_id(32個中何番目の故障なのか), ed_flag(FSIMイベントフラグ値), xid_flag(X抽出(ケアビット固定)フラグ値), priority(優先フラグ)
//------------------------------------------------------------------------------------
void	TDF_XID_pri_cost_fpath_and(S_NLIST *t_net, int tp_id, int fault_id, unsigned int ed_flag, unsigned int xid_flag, unsigned int jus_flag, unsigned int priority){

	int				i,j;
	int				in_id = -1;					//入力信号線の最小可制御性の入力ID
	unsigned int	in_cost = UNSIGNED_MUGEN;	//入力信号線の可制御性⇒最小の可制御性に更新
	int				in_pri_flag = NO;			//入力信号線に優先パスを発見したか{YES, NO}
			...
			if(t_net->in[i]->flag == ed_flag){
				if((t_net->in[i]->x_fault & MASKbit[fault_id]) == 0){	//AND入力故障値==0
					...
					if(Get_NBit(t_net->in[i]->xid_nval_t2, tp_id) == 3){	//ドントケア(X)発見
						...
						//優先パスフラグあり
						//--------------------------------
						if(t_net->in[i]->pri_path == priority){
							//まだ優先パスを発見してない場合⇒問答無用で上書き
							if(in_pri_flag == NO){
								in_cost = t_net->in[i]->cost[C_1][1];	//一番1可制御性が低い信号線を更新
								in_id = i;								//入力IDを保存
							}
							//すでに発見済みの場合⇒1可制御性が小さかったら上書き
							else{
						...
						//--------------------------------
							...
							//まだ優先パスを発見してない場合のみ最小1可制御性なら上書き
							if(in_pri_flag == NO){
								if(in_cost > t_net->cost[C_1][1]){
									in_cost = t_net->cost[C_1][1];	//一番1可制御性が低い信号線を更新
									in_id = i;						//入力IDを保存
								}
							}
						}
...
//------------------------------------------------------------------------------------
//  関数名 : TDF_XID_pri_cost_fpath_nand
//  機  能 : 【NAND】故障伝搬経路決定
//  戻り値 : なし
//  引  数 : t_net(対象信号線), tp_id(テストパターンの何番目か), fault_id(32個中何番目の故障なのか), ed_flag(FSIMイベントフラグ値), xid_flag(X抽出(ケアビット固定)フラグ値), priority(優先フラグ)
//------------------------------------------------------------------------------------
void	TDF_XID_pri_cost_fpath_nand(S_NLIST *t_net, int tp_id, int fault_id, unsigned int ed_flag, unsigned int xid_flag, unsigned int jus_flag, unsigned int priority){
...
//------------------------------------------------------------------------------------
//  関数名 : TDF_XID_pri_cost_fpath_or
//  機  能 : 【OR】故障伝搬経路決定
//  戻り値 : なし
//  引  数 : t_net(対象信号線), tp_id(テストパターンの何番目か), fault_id(32個中何番目の故障なのか), ed_flag(FSIMイベントフラグ値), xid_flag(X抽出(ケアビット固定)フラグ値), priority(優先フラグ)
//------------------------------------------------------------------------------------
void	TDF_XID_pri_cost_fpath_or(S_NLIST *t_net, int tp_id, int fault_id, unsigned int ed_flag, unsigned int xid_flag, unsigned int jus_flag, unsigned int priority){
				...
				if((t_net->in[i]->x_fault & MASKbit[fault_id]) != 0){	//OR入力故障値==1
					...
					if(Get_NBit(t_net->in[i]->xid_nval_t2, tp_id) == 3){	//ドントケア(X)発見
						...
						if(t_net->in[i]->pri_path == priority){
							//まだ優先パスを発見してない場合⇒問答無用で上書き
							if(in_pri_flag == NO){
								in_cost = t_net->in[i]->cost[C_0][1];	//一番0可制御性が低い信号線を更新
								in_id = i;								//入力IDを保存
							}
							//すでに発見済みの場合⇒1可制御性が小さかったら上書き
							else{
						...
						//--------------------------------
							...
							//まだ優先パスを発見してない場合のみ最小1可制御性なら上書き
							if(in_pri_flag == NO){
								if(in_cost > t_net->cost[C_0][1]){
									in_cost = t_net->cost[C_0][1];	//一番0可制御性が低い信号線を更新
									in_id = i;						//入力IDを保存
								}
							}
						}
...
//------------------------------------------------------------------------------------
//  関数名 : TDF_XID_pri_cost_fpath_nor
//  機  能 : 【NOR】故障伝搬経路決定
//  戻り値 : なし
//  引  数 : t_net(対象信号線), tp_id(テストパターンの何番目か), fault_id(32個中何番目の故障なのか), ed_flag(FSIMイベントフラグ値), xid_flag(X抽出(ケアビット固定)フラグ値), priority(優先フラグ)
//------------------------------------------------------------------------------------
void	TDF_XID_pri_cost_fpath_nor(S_NLIST *t_net, int tp_id, int fault_id, unsigned int ed_flag, unsigned int xid_flag, unsigned int jus_flag, unsigned int priority){
...
//------------------------------------------------------------------------------------
//  関数名 : TDF_XID_pri_cost_fpath_exor
//  機  能 : 【EXOR・EXNOR】故障伝搬経路決定
//  戻り値 : なし
//  引  数 : t_net(対象信号線), tp_id(テストパターンの何番目か), fault_id(32個中何番目の故障なのか), ed_flag(FSIMイベントフラグ値), xid_flag(X抽出(ケアビット固定)フラグ値), priority(優先フラグ)
//※2入力のみ対応
//------------------------------------------------------------------------------------
void	TDF_XID_pri_cost_fpath_exor(S_NLIST *t_net, int tp_id, int fault_id, unsigned int ed_flag, unsigned int xid_flag, unsigned int jus_flag, unsigned int priority){
