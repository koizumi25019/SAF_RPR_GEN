...
// プロトタイプ宣言
//---------------------------------------------------------------------
void		SAF_2v_spsfp_EDpush (NLIST*,int);
...
//----------------------------------------------
//  関数名 : SAF_2v_SPSFP
//  機  能 : 【2値演算(X-bufのみ計算)】縮退故障故障シミュレーション(SPSFP)
//  戻り値 : なし
//  引  数 : tp_id(テストパターンID, t_fault(故障信号線), fault_type(故障種類[SAF0 or SAF1]), ed_flag(イベントドリブン用フラグ)
//----------------------------------------------
void	SAF_2v_SPSFP(int tp_id, NLIST *t_fault, int fault_type, unsigned int ed_flag){
	...
	//-------------------------------
	SAF_2v_spsfp_EDpush(t_fault, ed_flag);
	...
	//-------------------------------
		...
		//対象信号線に故障値設置
		t_fault->x_fault = SAF1;
	}

#ifdef CAL_DEBUG
	printf("%s 故障値:%d\n",t_fault->name, t_fault->x_fault);
#endif
	...
	while(event_lev != (max_level+1) ){
...
#ifdef CAL_DEBUG
		printf("%s 故障値:%d\n",temp_net->name, temp_net->x_fault);
#endif
		...
		if(bit_nval == 0){
			//正常値≠故障値(故障伝搬)
			if(temp_net->x_fault == 1){	//故障値が1
				if(temp_net->n_out != 0){
					SAF_2v_spsfp_EDpush(temp_net, ed_flag);	//temp_netの出力をプッシュ
				}
...
#ifdef CAL_DEBUG
	printf("\n//-----------------------------------\n");
	printf("// DEBUG: PI正常値確認\n");
	printf("//-----------------------------------\n");
	for(i=0; i<n_pi; i++){
		printf("%s 正常値:%d\n", pi[i]->name, Get_NBit_Xbuf(pi[i]->nval, tp_id));
	}
...
//------------------------------------------------------------------------------------
static void	SAF_2v_ED_fout(NLIST *t_net, int tp_id, unsigned int ed_flag){
...
//  引  数 : t_net(対象信号線), tp_id(テストパターンの何番目か), ed_flag(現在のイベントフラグ値)
//------------------------------------------------------------------------------------
static void	SAF_2v_ED_buf(NLIST *t_net, int tp_id, unsigned int ed_flag){
...
//  引  数 : t_net(対象信号線), tp_id(テストパターンの何番目か), ed_flag(現在のイベントフラグ値)
//------------------------------------------------------------------------------------
static void	SAF_2v_ED_inv(NLIST *t_net, int tp_id, unsigned int ed_flag){
	
	//================================================
	// INV処理(否定演算)
	//================================================
	if(t_net->in[0]->x_fault == 0){
		t_net->x_fault = 1;
	}
	else if(t_net->in[0]->x_fault == 1){
		t_net->x_fault = 0;
	}
...
//  引  数 : t_net(対象信号線), tp_id(テストパターンの何番目か), ed_flag(現在のイベントフラグ値)
//------------------------------------------------------------------------------------
static void	SAF_2v_ED_and(NLIST *t_net, int tp_id, unsigned int ed_flag){
	...
	//-----------------------------------
		...
		//対象信号線のtp_id番目正常値を代入
		t_net->x_fault = Get_NBit_Xbuf(t_net->in[0]->nval, tp_id);
	...
	for(i=1; i<t_net->n_in; i++){
		...
		//-----------------------------------
			...
			//対象信号線のtp_id番目正常値を取得
			t_net->x_fault &= Get_NBit_Xbuf(t_net->in[i]->nval, tp_id);
		}
	}	
...
//  引  数 : t_net(対象信号線), tp_id(テストパターンの何番目か), ed_flag(現在のイベントフラグ値)
//------------------------------------------------------------------------------------
static void	SAF_2v_ED_nand(NLIST *t_net, int tp_id, unsigned int ed_flag){
...
//  引  数 : t_net(対象信号線), tp_id(テストパターンの何番目か), ed_flag(現在のイベントフラグ値)
//------------------------------------------------------------------------------------
static void	SAF_2v_ED_or(NLIST *t_net, int tp_id, unsigned int ed_flag){
...
//  引  数 : t_net(対象信号線), tp_id(テストパターンの何番目か), ed_flag(現在のイベントフラグ値)
//------------------------------------------------------------------------------------
static void	SAF_2v_ED_nor(NLIST *t_net, int tp_id, unsigned int ed_flag){
...
//※2入力のみ対応
//------------------------------------------------------------------------------------
static void	SAF_2v_ED_exor(NLIST *t_net, int tp_id, unsigned int ed_flag){
	...
	//===========================================================
		...
		t_net->x_fault = t_net->in[0]->x_fault ^ bit_nval;
	...
	//===========================================================
		...
		t_net->x_fault = bit_nval ^ t_net->in[1]->x_fault;
	}
	...
	//===========================================================
		...
		//EXOR出力信号線の正常値を取得
		t_net->x_fault = Get_NBit_Xbuf(t_net->nval, tp_id);
	}
}
...
//※2入力のみ対応
//------------------------------------------------------------------------------------
static void	SAF_2v_ED_exnor(NLIST *t_net, int tp_id, unsigned int ed_flag){
