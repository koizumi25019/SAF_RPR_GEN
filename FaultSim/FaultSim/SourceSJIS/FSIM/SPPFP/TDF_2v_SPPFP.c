...
void	TDF_2v_sppfp_ED_fout	(S_NLIST*, int, unsigned int);
void	TDF_2v_sppfp_ED_buf		(S_NLIST*, int, unsigned int);
void	TDF_2v_sppfp_ED_inv		(S_NLIST*, int, unsigned int);
void	TDF_2v_sppfp_ED_and		(S_NLIST*, int, unsigned int);
void	TDF_2v_sppfp_ED_nand	(S_NLIST*, int, unsigned int);
void	TDF_2v_sppfp_ED_or		(S_NLIST*, int, unsigned int);
void	TDF_2v_sppfp_ED_nor		(S_NLIST*, int, unsigned int);
void	TDF_2v_sppfp_ED_exor	(S_NLIST*, int, unsigned int);
void	TDF_2v_sppfp_ED_exnor	(S_NLIST*, int, unsigned int);
...
void	FPointer_TDF_2v_SPPFP(){
	...
	// 関数ポインタ作成
	//=========================================================
	sfunc_sppfp[FOUT]	= TDF_2v_sppfp_ED_fout;
	sfunc_sppfp[BUF]	= TDF_2v_sppfp_ED_buf;
	sfunc_sppfp[INV]	= TDF_2v_sppfp_ED_inv;
	sfunc_sppfp[AND]	= TDF_2v_sppfp_ED_and;
	sfunc_sppfp[NAND]	= TDF_2v_sppfp_ED_nand;
	sfunc_sppfp[OR]		= TDF_2v_sppfp_ED_or;
	sfunc_sppfp[NOR]	= TDF_2v_sppfp_ED_nor;
	sfunc_sppfp[EXOR]	= TDF_2v_sppfp_ED_exor;
	sfunc_sppfp[EXNOR]	= TDF_2v_sppfp_ED_exnor;
}
...
//----------------------------------------------
void	TDF_2v_SPPFP(int tp_id, SXID_LIST **t_fault, int n_fault, unsigned int ed_flag){
...
#ifdef TP_DEBUG
	printf("\n//==========================================\n");
	printf("// 2値初期テストパターン 遷移故障SPPFP\n");
	printf("//==========================================\n");
	printf("初期テストパターン[%d] 対象故障数%d\n", tp_id, n_fault);

	for(i=0; i<n_pi; i++)printf("%d", Get_NBit_Xbuf(spi[i]->nval_t1, tp_id));
	for(i=0; i<n_ppi; i++)printf("%d", Get_NBit_Xbuf(ppi[i]->nval_t1, tp_id));
	printf("\n");

	for(i=0; i<n_snet; i++){
		printf("%s xf: ",s_nl[i].name);
		dtob(s_nl[i].x_fault, 8);
	}
	printf("\n");
...
#ifdef T2_DEBUG
	printf("\n//==========================================\n");
	printf("// 2値二次刻目論理値 遷移故障SPPFP\n");
	printf("//==========================================\n");
	printf("テストパターン%d番目\n", tp_id);
	for(i=0; i<n_snet; i++)printf("%s(%d) %d\n", s_nl[i].name, s_nl[i].type,  Get_NBit_Xbuf(s_nl[i].nval_t2, tp_id));
	printf("\n");
	...
	for(i=0; i<n_fault; i++){
...
#ifdef FSET_DEBUG
		printf("故障[%d]:%s (type:%d) ", i, t_fault[i]->net->name, t_fault[i]->net->type);
		...
		if(t_fault[i]->net->n_out>0 && t_fault[i]->net->ppo_flag==NO){		//POでない&&PPOでない
			TDF_EDpush(t_fault[i]->net, ed_flag);							//故障信号線の出力にflag= ed_flag-1
		}
		...
		if(t_fault[i]->net->ppo_flag==YES && t_fault[i]->net->flag != (ed_flag-1) ){
			...
			for(j=0; j<t_fault[i]->net->n_in; j++){
				if(t_fault[i]->net->in[j]->flag != (ed_flag-1) ){
					//入力の正常値:0
					if(Get_NBit_Xbuf(t_fault[i]->net->in[j]->nval_t2, tp_id) == 0){
	...
	while(event_lev != (max_level+1) ){
		...
		//-------------------------------------------
		//対象信号線の正常値を取得
		bit_nval = Get_NBit_Xbuf(temp_net->nval_t2, tp_id);
	...
	for(i=0; i<n_ppo; i++){
		printf("%s\n",ppo[i]->name);
		bit_nval = Get_NBit_Xbuf(ppo[i]->nval_t2, tp_id);
		if(bit_nval == 0){
			printf(" nval: 0\n");
		}
		else if(bit_nval == 1){
...
//******************************************************************************************************
//------------------------------------------------------------------------------------
//  関数名 : TDF_2v_sppfp_ED_fout
//  機  能 : 【FOUT】2値遷移故障イベントドリブン(故障値計算)
//  戻り値 : なし
//  引  数 : t_net(対象信号線), tp_id(テストパターンの何番目か), ed_flag(現在のイベントフラグ値)
//------------------------------------------------------------------------------------
void	TDF_2v_sppfp_ED_fout(S_NLIST *t_net, int tp_id, unsigned int ed_flag){
...
//------------------------------------------------------------------------------------
//  関数名 : TDF_2v_sppfp_ED_buf
//  機  能 : 【BUF】2値遷移故障イベントドリブン(故障値計算)
//  戻り値 : なし
//  引  数 : t_net(対象信号線), tp_id(テストパターンの何番目か), ed_flag(現在のイベントフラグ値)
//------------------------------------------------------------------------------------
void	TDF_2v_sppfp_ED_buf(S_NLIST *t_net, int tp_id, unsigned int ed_flag){
...
//------------------------------------------------------------------------------------
//  関数名 : TDF_2v_sppfp_ED_inv
//  機  能 : 【INV】2値遷移故障イベントドリブン(故障値計算)
//  戻り値 : なし
//  引  数 : t_net(対象信号線), tp_id(テストパターンの何番目か), ed_flag(現在のイベントフラグ値)
//------------------------------------------------------------------------------------
void	TDF_2v_sppfp_ED_inv(S_NLIST *t_net, int tp_id, unsigned int ed_flag){
...
//------------------------------------------------------------------------------------
//  関数名 : TDF_2v_sppfp_ED_and
//  機  能 : 【AND】2値遷移故障イベントドリブン(故障値計算)
//  戻り値 : なし
//  引  数 : t_net(対象信号線), tp_id(テストパターンの何番目か), ed_flag(現在のイベントフラグ値)
//------------------------------------------------------------------------------------
void	TDF_2v_sppfp_ED_and(S_NLIST *t_net, int tp_id, unsigned int ed_flag){
	...
	//-----------------------------------
		...
		//対象信号線のtp_id番目正常値を取得
		bit_nval = Get_NBit_Xbuf(t_net->in[0]->nval_t2, tp_id);
	...
	for(i=1; i<t_net->n_in; i++){
		...
		//-----------------------------------
			...
			//対象信号線のtp_id番目正常値を取得
			bit_nval = Get_NBit_Xbuf(t_net->in[i]->nval_t2, tp_id);
...
//------------------------------------------------------------------------------------
//  関数名 : TDF_2v_sppfp_ED_nand
//  機  能 : 【NAND】2値遷移故障イベントドリブン(故障値計算)
//  戻り値 : なし
//  引  数 : t_net(対象信号線), tp_id(テストパターンの何番目か), ed_flag(現在のイベントフラグ値)
//------------------------------------------------------------------------------------
void	TDF_2v_sppfp_ED_nand(S_NLIST *t_net, int tp_id, unsigned int ed_flag){
...
//------------------------------------------------------------------------------------
//  関数名 : TDF_2v_sppfp_ED_or
//  機  能 : 【OR】2値遷移故障イベントドリブン(故障値計算)
//  戻り値 : なし
//  引  数 : t_net(対象信号線), tp_id(テストパターンの何番目か), ed_flag(現在のイベントフラグ値)
//------------------------------------------------------------------------------------
void	TDF_2v_sppfp_ED_or(S_NLIST *t_net, int tp_id, unsigned int ed_flag){
...
//------------------------------------------------------------------------------------
//  関数名 : TDF_2v_sppfp_ED_nor
//  機  能 : 【NOR】2値遷移故障イベントドリブン(故障値計算)
//  戻り値 : なし
//  引  数 : t_net(対象信号線), tp_id(テストパターンの何番目か), ed_flag(現在のイベントフラグ値)
//------------------------------------------------------------------------------------
void	TDF_2v_sppfp_ED_nor(S_NLIST *t_net, int tp_id, unsigned int ed_flag){
...
//------------------------------------------------------------------------------------
//  関数名 : TDF_2v_sppfp_ED_exor
//  機  能 : 【EXOR】2値遷移故障イベントドリブン(故障値計算)
//  戻り値 : なし
//  引  数 : t_net(対象信号線), tp_id(テストパターンの何番目か), ed_flag(現在のイベントフラグ値)
//※2入力のみ対応
//------------------------------------------------------------------------------------
void	TDF_2v_sppfp_ED_exor(S_NLIST *t_net, int tp_id, unsigned int ed_flag){
	...
	//------------------------------------------------
		...
		//1番目正常値を取得
		bit_nval = Get_NBit_Xbuf(t_net->in[1]->nval_t2, tp_id);
	...
	//------------------------------------------------
		...
		//EXOR出力信号線の正常値を取得
		bit_nval = Get_NBit_Xbuf(t_net->nval_t2, tp_id);
...
//------------------------------------------------------------------------------------
//  関数名 : TDF_2v_sppfp_ED_exnor
//  機  能 : 【EXNOR】2値遷移故障イベントドリブン(故障値計算)
//  戻り値 : なし
//  引  数 : t_net(対象信号線), tp_id(テストパターンの何番目か), ed_flag(現在のイベントフラグ値)
//※2入力のみ対応
//------------------------------------------------------------------------------------
void	TDF_2v_sppfp_ED_exnor(S_NLIST *t_net, int tp_id, unsigned int ed_flag){
