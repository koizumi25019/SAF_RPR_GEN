...
void	SAF_3v_ED_fout		(NLIST*, int, unsigned int);
void	SAF_3v_ED_buf		(NLIST*, int, unsigned int);
void	SAF_3v_ED_inv		(NLIST*, int, unsigned int);
void	SAF_3v_ED_and		(NLIST*, int, unsigned int);
void	SAF_3v_ED_nand		(NLIST*, int, unsigned int);
void	SAF_3v_ED_or		(NLIST*, int, unsigned int);
void	SAF_3v_ED_nor		(NLIST*, int, unsigned int);
void	SAF_3v_ED_exor		(NLIST*, int, unsigned int);
void	SAF_3v_ED_exnor		(NLIST*, int, unsigned int);
...
//------------------------------------------------------------------------
//----------------------------------------------
//  関数名 : Make_FP_SAF_3v_ED_PPSFP
//  機  能 : 【関数ポインタ作成】STEMからの3値縮退故障PPSFPイベントドリブン
//  戻り値 : なし
//  引  数 : なし
//----------------------------------------------
void Make_FP_SAF_3v_ED_PPSFP(){
	...
	// 関数ポインタ作成
	//=========================================================
	func_ed_ppsfp[FOUT]	 = SAF_3v_ED_fout;
	func_ed_ppsfp[BUF]	 = SAF_3v_ED_buf;
	func_ed_ppsfp[INV]	 = SAF_3v_ED_inv;
	func_ed_ppsfp[AND]	 = SAF_3v_ED_and;
	func_ed_ppsfp[NAND]	 = SAF_3v_ED_nand;
	func_ed_ppsfp[OR]	 = SAF_3v_ED_or;
	func_ed_ppsfp[NOR]	 = SAF_3v_ED_nor;
	func_ed_ppsfp[EXOR]	 = SAF_3v_ED_exor;
	func_ed_ppsfp[EXNOR] = SAF_3v_ED_exnor;
}
...
//----------------------------------------------
void	SAF_3v_ED_PPSFP (int ffr_id, NLIST* stem_net, unsigned int ed_flag, int ui_num){
...
//------------------------------------------------------------------------------------
void	SAF_3v_ED_fout(NLIST *t_net, int ui_num, unsigned int ed_flag){
...
//  引  数 : t_net(対象信号線), ui_num(unsigned intの何番目か), ed_flag(現在のイベントフラグ値)
//------------------------------------------------------------------------------------
void	SAF_3v_ED_buf(NLIST *t_net, int ui_num, unsigned int ed_flag){
...
//  引  数 : t_net(対象信号線), ui_num(unsigned intの何番目か), ed_flag(現在のイベントフラグ値)
//------------------------------------------------------------------------------------
void	SAF_3v_ED_inv(NLIST *t_net, int ui_num, unsigned int ed_flag){
...
//  引  数 : t_net(対象信号線), ui_num(unsigned intの何番目か), ed_flag(現在のイベントフラグ値)
//------------------------------------------------------------------------------------
void	SAF_3v_ED_and(NLIST *t_net, int ui_num, unsigned int ed_flag){
	...
	//-----------------------------------
		...
		t_net->p_fault = t_net->in[0]->nval->p_buf[ui_num];
	}
	...
	for(i=1; i<t_net->n_in; i++){
		...
		//-----------------------------------
			...
			t_net->p_fault |= t_net->in[i]->nval->p_buf[ui_num];
		}
	}
...
//  引  数 : t_net(対象信号線), ui_num(unsigned intの何番目か), ed_flag(現在のイベントフラグ値)
//------------------------------------------------------------------------------------
void	SAF_3v_ED_nand(NLIST *t_net, int ui_num, unsigned int ed_flag){
...
//  引  数 : t_net(対象信号線), ui_num(unsigned intの何番目か), ed_flag(現在のイベントフラグ値)
//------------------------------------------------------------------------------------
void	SAF_3v_ED_or(NLIST *t_net, int ui_num, unsigned int ed_flag){
...
//  引  数 : t_net(対象信号線), ui_num(unsigned intの何番目か), ed_flag(現在のイベントフラグ値)
//------------------------------------------------------------------------------------
void	SAF_3v_ED_nor(NLIST *t_net, int ui_num, unsigned int ed_flag){
...
//※2入力のみ対応
//------------------------------------------------------------------------------------
void	SAF_3v_ED_exor(NLIST *t_net, int ui_num, unsigned int ed_flag){
	...
	//------------------------------------------------
		...
		t_net->x_fault = (t_net->in[0]->x_fault & t_net->in[1]->nval->p_buf[ui_num]) | (t_net->in[0]->p_fault & t_net->in[1]->nval->x_buf[ui_num]);
		t_net->p_fault = (t_net->in[0]->p_fault | t_net->in[1]->nval->x_buf[ui_num]) & (t_net->in[0]->x_fault | t_net->in[1]->nval->p_buf[ui_num]);
	}
	...
	//------------------------------------------------
		...
		t_net->x_fault = (t_net->in[0]->nval->x_buf[ui_num] & t_net->in[1]->p_fault) | (t_net->in[0]->nval->p_buf[ui_num] & t_net->in[1]->x_fault);
		t_net->p_fault = (t_net->in[0]->nval->p_buf[ui_num] | t_net->in[1]->x_fault) & (t_net->in[0]->nval->x_buf[ui_num] | t_net->in[1]->p_fault);
	}
	...
	//------------------------------------------------
		...
		t_net->x_fault = (t_net->in[0]->nval->x_buf[ui_num] & t_net->in[1]->nval->p_buf[ui_num]) | (t_net->in[0]->nval->p_buf[ui_num] & t_net->in[1]->nval->x_buf[ui_num]);
		t_net->p_fault = (t_net->in[0]->nval->p_buf[ui_num] | t_net->in[1]->nval->x_buf[ui_num]) & (t_net->in[0]->nval->x_buf[ui_num] | t_net->in[1]->nval->p_buf[ui_num]);
	}
...
//※2入力のみ対応
//------------------------------------------------------------------------------------
void	SAF_3v_ED_exnor(NLIST *t_net, int ui_num, unsigned int ed_flag){
