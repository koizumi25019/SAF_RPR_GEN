//------------------------------------------------------------------------
//File name : SAF_2v_ED_PPSFP_Ndrop.c
//Date : 2012/2/1
//Designer : H.Yamazaki
//Ver : 0.01
//------------------------------------------------------------------------
...
void	FOUT_2v_CPT_Ndrop	(int, NLIST*, int, unsigned int*, NLIST*);
...
//----------------------------------------------
//  関数名 : SAF_2v_ED_PPSFP_Ndrop
//  機  能 : STEMからの2値縮退故障PPSFPイベントドリブン
//  戻り値 : なし
//  引  数 : ffr_id(ステムのFFR番号), stem_net(ステム信号線), ed_flag(計算済みフラグ値), ui_num(unsigned int番目)
//----------------------------------------------
void	SAF_2v_ED_PPSFP_Ndrop (int ffr_id, NLIST* stem_net, unsigned int ed_flag, int ui_num){
	...
	while(event_lev != (max_level+1) ){
		...
		if(temp_net->nval->x_buf[ui_num] != temp_net->x_fault){
			...
			//--------------------------------------------------------------------
			if(temp_net->n_out == 0){
				FOUT_2v_CPT_Ndrop(ffr_id, stem_net, ui_num, &det_tp_list, temp_net);
