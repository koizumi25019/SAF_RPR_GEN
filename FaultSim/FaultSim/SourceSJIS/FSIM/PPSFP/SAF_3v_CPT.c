...
//------------------------------------------------------------------------------------
void	SAF_3v_CPT0(int ffr_id, unsigned int tp_id, NLIST *t_net){
	...
	switch(t_net->type){
		...
		case BUF:
			...
			//さらに入力信号線をCPT
			SAF_3v_CPT0(ffr_id, tp_id, t_net->in[0]);
		...
		case INV:
			...
			//さらに入力信号線をCPT
			SAF_3v_CPT1(ffr_id, tp_id, t_net->in[0]);
		...
		case AND:
			...
			if(n_contorolling==1 && n_xbit==0){
				...
				//入力信号線のさらに入力信号線をCPT
				SAF_3v_CPT0(ffr_id, tp_id, t_net->in[in]);
			}
		...
		case NAND:
			...
			for(i=0; i<t_net->n_in; i++){
				...
				//さらに入力信号線をCPT
				SAF_3v_CPT1(ffr_id, tp_id, t_net->in[i]);
			}
		...
		case OR:
			...
			for(i=0; i<t_net->n_in; i++){
				...
				//さらに入力信号線をCPT
				SAF_3v_CPT0(ffr_id, tp_id, t_net->in[i]);
			}
		...
		case NOR:
			...
			if(n_contorolling==1 && n_xbit==0){
				...
				//入力信号線のさらに入力信号線をCPT
				SAF_3v_CPT1(ffr_id, tp_id, t_net->in[in]);
			}
...
//  引  数 : ffr_id(FFR番号), tp_id(テストパターンの何番目か), t_net(CPTを始める信号線[=FFRの先頭])
//------------------------------------------------------------------------------------
void	SAF_3v_CPT1(int ffr_id, unsigned int tp_id, NLIST *t_net){
