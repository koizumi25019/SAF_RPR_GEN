...
// プロトタイプ宣言
//---------------------------------------------------------------------
void	TDF_2v_CPT0_Ndrop		(int, unsigned int, S_NLIST*);
void	TDF_2v_CPT1_Ndrop		(int, unsigned int, S_NLIST*);
void	TDF_3v_CPT0_Ndrop		(int, unsigned int, S_NLIST*);
void	TDF_3v_CPT1_Ndrop		(int, unsigned int, S_NLIST*);
void	TDF_2v_ED_PPSFP_Ndrop	(int, S_NLIST*, unsigned int, int);
void	TDF_3v_ED_PPSFP_Ndrop	(int, S_NLIST*, unsigned int, int);
...
void	TDF_2v_PPSFP_Ndrop(){
	...
	// 故障シミュレーション
	//====================================================
	fprintf(stderr,"\n\n FSIM_2v_sequential(PPSFP %ddrop)\n", opt.n_drop);
	fprintf(stderr,"|----+----|----+----|----+----|----+----|----+----|\n");
		...
		for(j=0; (int)j<n_tp; j++){
			...
			if(nval_t2 == 0){
				...
				if(nval_t1 == 1){
					...
					if(sffr[i].FoutStem->det_stf<opt.n_drop && sffr[i].FoutStem->test_stf==YES){
				...
				//2時刻目正常値=0からのCPT開始
				TDF_2v_CPT0_Ndrop(i, j, sffr[i].FoutStem);
			}
			...
			//------------------------------------
				...
				if(nval_t1 == 0){
					...
					//検出回数がopt.n_drop回以下なら検出回数等を保存
					if(sffr[i].FoutStem->det_str<opt.n_drop && sffr[i].FoutStem->test_str==YES){
						sffr[i].n_detect++;									//FFR内の検出故障数更新
				...
				//2時刻目正常値=1からのCPT開始
				TDF_2v_CPT1_Ndrop(i, j, sffr[i].FoutStem);
			}

			//------------------------------------
			//★対象FFR内の全故障をopt.n_drop回数以上検出したらbreak!!
			//------------------------------------
			if(sffr[i].n_detect == sffr[i].n_fault *opt.n_drop){
#ifdef BREAK
				n_break++;
#endif
				break;
	...
	for( ; i<n_ffr; i++){ //初期化式にiは使われないため，iを空欄に変更
		...
		if(sffr[i].FoutStem->testable_net == YES){
			...
			for(j=0; j<n_tp_int; j++){	//確保したunsigned int数分ループ			
				...
				//PPSFPイベントドリブン開始
				TDF_2v_ED_PPSFP_Ndrop(i, sffr[i].FoutStem, ed_flag, j);	//2値PPSFPイベントドリブン
...
void	TDF_3v_PPSFP_Ndrop(){
	...
	// 故障シミュレーション
	//====================================================
	fprintf(stderr,"\n\n FSIM_3v_sequential(PPSFP %ddrop)\n", opt.n_drop);
	fprintf(stderr,"|----+----|----+----|----+----|----+----|----+----|\n");
	...
	for(i=n_spo; i<(n_spo+n_ppo); i++){
		...
		for(j=0; (int)j<n_tp; j++){
			...
			if(nval_t2 == 0){
				...
				//2時刻目正常値=0からのCPT開始
				TDF_3v_CPT0_Ndrop(i, j, sffr[i].FoutStem);
			}
			...
			//------------------------------------
				...
				//2時刻目正常値=1からのCPT開始
				TDF_3v_CPT1_Ndrop(i, j, sffr[i].FoutStem);
			}
	...
	for( ; i<n_ffr; i++){//初期化式にiは使われないため，iを空欄に変更
		...
		if(sffr[i].FoutStem->testable_net == YES){
			...
			for(j=0; j<n_tp_int; j++){	//確保したunsigned int数分ループ			
				...
				//PPSFPイベントドリブン開始
				TDF_3v_ED_PPSFP_Ndrop(i, sffr[i].FoutStem, ed_flag, j);	//2値PPSFPイベントドリブン
