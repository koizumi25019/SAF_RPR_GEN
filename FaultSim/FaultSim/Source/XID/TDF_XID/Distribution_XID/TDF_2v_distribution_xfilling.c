...
unsigned int	TDF_2v_distribution_xfilling(int *tp_id, int tp_count, SXID_LIST* fnet, unsigned int ed_flag, unsigned int xid_flag, unsigned int *jus_flag){
	...
	int			i,j;//,k; kは未使用変数のため削除
//	int			event_lev;					//故障伝搬経路を決めるレベル //event_levは未使用変数のため削除
//	int			level_flag=0;				//event_lev更新時にbreakしないようにする //level_flagは未使用変数のため削除
//	S_NLIST		*temp_net=NULL;				//スタックから取り出した信号線を一時的に格納	//*temp_netは未使用変数のため削除
	...
	return 0; //関数の宣言がvoidでないため，return 0を追加
}
