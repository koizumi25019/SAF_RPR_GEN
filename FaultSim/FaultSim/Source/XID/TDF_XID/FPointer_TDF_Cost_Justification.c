//------------------------------------------------------------------------
//File name : FPointer_TDF_Justification.c
//Date : 2013/10/17
//Designer : H.Yamazaki
//Ver : 0.01
//------------------------------------------------------------------------
...
//限定正当化
void	TDF_XID_cost_jus_and_nand			(S_NLIST*, int, unsigned int, unsigned int);
void	TDF_XID_cost_jus_or_nor				(S_NLIST*, int, unsigned int, unsigned int);
...
//------------------------------------------------------------------------
//----------------------------------------------
//  関数名 : FPointer_TDF_Cost_Justification
...
void	FPointer_TDF_Cost_Justification(){
	...
	//======================================================================================
	//限定正当化
	sfunc_jus[AND]	= TDF_XID_cost_jus_and_nand;	//ANDとNANDは同じ処理
	sfunc_jus[NAND]	= TDF_XID_cost_jus_and_nand;	//ANDとNANDは同じ処理
	sfunc_jus[OR]	= TDF_XID_cost_jus_or_nor;		//ORとNORは同じ処理
	sfunc_jus[NOR]	= TDF_XID_cost_jus_or_nor;		//ORとNORは同じ処理
}
...
//★限定正当化★
//------------------------------------------------------------------------------------
//  関数名 : TDF_XID_cost_jus_and_nand
//  機  能 : 【AND・NAND】2値・3値限定正当化
//  戻り値 : なし
//  引  数 : t_net(対象信号線), tp_id(テストパターンの何番目か), xid_flag(現在のケアビット固定フラグ値), jus_flag(限定正当化フラグ)
//------------------------------------------------------------------------------------
void	TDF_XID_cost_jus_and_nand(S_NLIST *t_net, int tp_id, unsigned int xid_flag, unsigned int jus_flag){
				...
				if(Get_NBit_Xbuf(t_net->in[i]->nval_t1, tp_id) == 0){
					...
					//xid_nvalが『X』の場合
						...
						if(in_cost > t_net->in[i]->cost[C_0][0]){
							in_cost = t_net->in[i]->cost[C_0][0];	//一番可制御性が低い信号線を更新
							in_id = i;								//入力IDを保存
						}
					}
				...
				if(Get_NBit_Xbuf(t_net->in[i]->nval_t2, tp_id) == 0){
					...
					//xid_nvalが『X』の場合
						...
						if(in_cost > t_net->in[i]->cost[C_0][1]){
							in_cost = t_net->in[i]->cost[C_0][1];	//一番可制御性が低い信号線を更新
							in_id = i;								//入力IDを保存
						}
					}
...
//------------------------------------------------------------------------------------
//  関数名 : TDF_XID_cost_jus_or_nor
//  機  能 : 【OR・NOR】2値・3値限定正当化
//  戻り値 : なし
//  引  数 : t_net(対象信号線), tp_id(テストパターンの何番目か), xid_flag(現在のケアビット固定フラグ値), jus_flag(限定正当化フラグ)
//------------------------------------------------------------------------------------
void	TDF_XID_cost_jus_or_nor(S_NLIST *t_net, int tp_id, unsigned int xid_flag, unsigned int jus_flag){

	int				i;
	int				in_id;						//入力信号線の最小可制御性の入力ID
	unsigned int	in_cost = UNSIGNED_MUGEN;	//入力信号線の可制御性⇒最小可制御性に更新
	int				buff_nval;					//正常値の一時取得
				...
				if(Get_NBit_Xbuf(t_net->in[i]->nval_t1, tp_id) == 1){
					...
					//xid_nvalが『X』の場合
						...
						if(in_cost > t_net->in[i]->cost[C_1][0]){
							in_cost = t_net->in[i]->cost[C_1][0];	//一番可制御性が低い信号線を更新
							in_id = i;								//入力IDを保存
						}
					}
				...
				if(Get_NBit_Xbuf(t_net->in[i]->nval_t2, tp_id) == 1){
					...
					//xid_nvalが『X』の場合
						...
						if(in_cost > t_net->in[i]->cost[C_1][1]){
							in_cost = t_net->in[i]->cost[C_1][1];	//一番可制御性が低い信号線を更新
							in_id = i;								//入力IDを保存
						}
					}
