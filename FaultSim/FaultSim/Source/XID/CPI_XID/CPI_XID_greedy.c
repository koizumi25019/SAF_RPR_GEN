...
void	CPI_XID_greedy(){
	...
	for(i=0; i<n_cpi; i++){
		...
		for(j=0; j<n_ffc; j++){
			...
			if(decision_flag[j] == NO){
				...
				//X抽出率
				fprintf(log, "%.2lf%%	", ((double)n_after_x/(double)(n_tp*(n_spi+n_ppi)))*100); //%.2lf%を%.2lf%%に変更
	...
	//ファイルオープン
	if((tp = fopen("tp_temp.txt","w")) == (FILE *)NULL){
		fprintf(stderr,"Cannot open TP_temp file %s\n", "tp_temp.txt"); //%sの引数として，"tp_temp.txt"を追加
		exit(1);
	}
