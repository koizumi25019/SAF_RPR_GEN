//------------------------------------------------------------------------
//File name : TDF_priority_m2004_XID.c
//Date : 2013/11/22
//Designer : H.Yamazaki
//Ver : 0.01
//------------------------------------------------------------------------
...
//------------------------------------------------------------------------
//----------------------------------------------
//  関数名 : TDF_priroity_m2004_XID
//  機  能 : ★優先パスモード【遷移故障】Miyase2004ドントケア抽出
//  戻り値 : なし
//  引  数 : t_fault(X抽出対象故障リストの空の箱), priroty(優先パスフラグ)
//----------------------------------------------
void	TDF_priority_m2004_XID(SXID_LIST **t_fault, unsigned int priority){
	...
	for(i=0; i<n_tp; i++){
		...
		for(j=0; j<n_snet; j++){
			...
			if(flt_stbl[j]->test_str==YES && flt_stbl[j]->det_str>1 && flt_stbl[j]->xid_det_str==NO && flt_stbl[j]->n_str_xid<opt.n_xid){	//対象故障かつ1回以上検出かつ未検出？
				...
				if( Get_NBit_INT(fdic_str[sort_tp[i]->tp_id], flt_stbl[j]->n)==YES ){	//対象TPで検出するか？
#ifdef DEBUG
					printf("STR %s\n", flt_stbl[j]->name);
#endif
					//テストパターンごとのXID故障数更新
					tp_info[sort_tp[i]->tp_id].n_xid_fault++;
					...
					if(count == 31){
						...
						if(n_before_x == 0){
							//SPPFP故障SIM
							TDF_2v_SPPFP(sort_tp[i]->tp_id, t_fault, count, ed_flag);
							
							//優先パス2値XID
							xid_flag = TDF_priority_2v_xfilling(sort_tp[i]->tp_id, t_fault, count, ed_flag, xid_flag, &jus_flag, priority);
						}
						//【3値FSIM】&【X抽出】
						else{
							//SPPFP故障SIM
							TDF_3v_SPPFP(sort_tp[i]->tp_id, t_fault, count, ed_flag);
							
							//優先パス3値XID
							xid_flag = TDF_priority_3v_xfilling(sort_tp[i]->tp_id, t_fault, count, ed_flag, xid_flag, &jus_flag, priority);
						...
						//----------------------------------------------
						n_t_fault -= TDF_3v_XID_SPPFP_FSIM(sort_tp[i]->tp_id, &ed_flag, t_fault);		//X抽出対象故障を更新
			...
			if(flt_stbl[j]->test_stf==YES && flt_stbl[j]->det_stf>1 && flt_stbl[j]->xid_det_stf==NO && flt_stbl[j]->n_stf_xid<opt.n_xid){	//対象故障かつ1回以上検出かつ未検出？
				...
				if( Get_NBit_INT(fdic_stf[sort_tp[i]->tp_id], flt_stbl[j]->n)==YES ){	//対象TPで検出するか？
#ifdef DEBUG
					printf("STF %s\n", flt_stbl[j]->name);
#endif					
					//テストパターンごとのXID故障数更新
	...
	*/
	...
	//===============================================
	// DEBUG：Miyase2004後のテストパターン確認
	//===============================================
#ifdef DEBUG
	printf("\n//======================================\n");
	printf("// Miyase2004後のテストパターン確認\n");
	printf("//======================================\n");
	for(i=0; i<n_tp; i++){
		printf("tp[%d]\t", i);
