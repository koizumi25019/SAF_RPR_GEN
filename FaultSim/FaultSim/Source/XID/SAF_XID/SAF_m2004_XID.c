...
//----------------------------------------------
//  関数名 : SAF_m2004_XID
//  機  能 : 【縮退故障】Miyase2004ドントケア抽出
//  戻り値 : なし
//  引  数 : t_fault(X抽出対象故障リストの空の箱)
//----------------------------------------------
void	SAF_m2004_XID(XID_LIST	**t_fault){
	...
	for(i=0; i<n_tp; i++){
		...
		for(j=0; j<n_net; j++){
			...
			// ■0縮退故障の対象故障判定
			//====================================================================================
			if(nl[j].test_sf0==YES && nl[j].det_sf0>1 && nl[j].xid_det_sf0==NO && nl[j].n_sa0_xid<opt.n_xid){	//対象故障かつ1回以上検出かつ未検出？
			...
			// ■1縮退故障の対象故障判定
			//====================================================================================
			if(nl[j].test_sf1==YES && nl[j].det_sf1>1 && nl[j].xid_det_sf1==NO && nl[j].n_sa1_xid<opt.n_xid){	//対象故障かつ1回以上検出かつ未検出？
