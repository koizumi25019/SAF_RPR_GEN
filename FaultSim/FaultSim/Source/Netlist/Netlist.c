...
static void read_1st(FILE *rfp)
{
	...
	while(get_token(rfp, str, 1, 0) != EOF) {
		...
		if(!strcmp(str, "module")) {
			...
			while((c = getc(rfp)) != ';'){} continue; //空ループの表現を;から{}に置換
		}
		else if(!strcmp(str, "wire")) {while((c = getc(rfp)) != ';'){} continue;} //空ループの表現を;から{}に置換
		//assign数算出
...
static void read_2nd(FILE *rfp)
{
	...
	while(get_token(rfp, str, 2, &in_num) != EOF) {
		if(!strcmp(str, "module")) {while((c = getc(rfp)) != ';'){} continue;} //空ループの表現を;から{}に置換
		else if(!strcmp(str, "input")) {while((c = getc(rfp)) != ';'){} continue;} //空ループの表現を;から{}に置換
		else if(!strcmp(str, "output")) {while((c = getc(rfp)) != ';'){} continue;} //空ループの表現を;から{}に置換
		else if(!strcmp(str, "wire")) {while((c = getc(rfp)) != ';'){} continue;} //空ループの表現を;から{}に置換
		else if(!strcmp(str, "assign")) {while((c = getc(rfp)) != ';'){} continue;} //空ループの表現を;から{}に置換
		else if(!strcmp(str, "endmodule")) {continue;}
		else {
			//入力数の読み込み
