//------------------------------------------------------------------------
//Netlist
//File name : s_Netlist.c
//Date : 2010/06/11
//Designer : R.Inoue(H.Yamazaki改)
//Ver : 3.03（配布Ver）
//------------------------------------------------------------------------
...
char s_pin_name_array[10] = { 'A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J' };
...
static void analysis_asyntax_error(void);
...
static void read_1st(FILE *rfp)
{
	...
	while(get_token(rfp, str, 1, 0) != EOF) {
		...
		if(!strcmp(str, "module")) {
			...
			smodule_name = (char *)malloc(sizeof(char) * (strlen(str) + 1));
			strcpy(smodule_name, str);
			while((c = getc(rfp)) != ';'){} continue; //空ループの明示として，;ではなく{}が推奨されています．
		}
		else if(!strcmp(str, "wire")) {while((c = getc(rfp)) != ';'){} continue;} //空ループの明示として，;ではなく{}が推奨されています．
		//assign数算出
...
static void calc_pi_po(FILE *rfp, int mode)
{
	...
	if(c == '[') {
		...
		if(re_no == 1) num ++;
		if(mode ==0) n_spi += (num * (abs(bit_1 - bit_2) + 1));
		else if(mode == 1) n_spo += (num * (abs(bit_1 - bit_2) + 1));
	}
	//bit幅なし
	else {
		ungetc(c, rfp);
		...
		while((re_no!=1) && (re_no!=2)) {
			...
			if((re_no==0) && (mode==0)) n_spi ++;
            else if((re_no==0) && (mode==1)) n_spo ++;
			re_no = get_token(rfp, str, 0, 0);
		}
		//クロック読み飛ばし（クロックでなっかた場合実行）
		if(strcmp(str, "CLK") && strcmp(str, "clk")) {
			if((re_no==1) && (mode==0)) n_spi ++;
			else if((re_no==1) && (mode==1)) n_spo ++;
		}
	}
}
...
static void read_2nd(FILE *rfp)
{
	...
	while(get_token(rfp, str, 2, &in_num) != EOF) {
		if(!strcmp(str, "module")) {while((c = getc(rfp)) != ';'){} continue;} //空ループの明示として，;ではなく{}が推奨されています．
		else if(!strcmp(str, "input")) {while((c = getc(rfp)) != ';'){} continue;} //空ループの明示として，;ではなく{}が推奨されています．
		else if(!strcmp(str, "output")) {while((c = getc(rfp)) != ';'){} continue;} //空ループの明示として，;ではなく{}が推奨されています．
		else if(!strcmp(str, "wire")) {while((c = getc(rfp)) != ';'){} continue;} //空ループの明示として，;ではなく{}が推奨されています．
		else if(!strcmp(str, "assign")) {while((c = getc(rfp)) != ';'){} continue;} //空ループの明示として，;ではなく{}が推奨されています．
		else if(!strcmp(str, "endmodule")) {continue;}
		else {
			//入力数の読み込み
...
static void read_module_type(FILE *rfp, char *str, int m_no)
{
	switch (str[0]) {
		...
		case 'D': if(strlen(str) == 3) {module_array[m_no]->m_type = DFF; n_sdff ++;}
				  else {module_array[m_no]->m_type = DFFS; n_sdffs ++;}
				  break;
		...
		case 'R': if(strlen(str) == 4) {module_array[m_no]->m_type = RDFF; n_srdff ++;}
				  else {module_array[m_no]->m_type = RDFFS; n_srdffs ++;}
				  break;
	}
}
...
static void read_pin(FILE *rfp, int m_no)
{
	...
	for(i=0; i<module_array[m_no]->m_n_in+1; i++) { //出力線分+1
		...
		//その他の入力信号
			...
			j = 0;
			while(s_pin_name_array[j] != str[0]) {
				j++;
...
static void alloc_nl(void)
{
	...
	//仮の信号線数
	nl_num = n_spi + n_spo + n_module;
	for(i=0; i<n_module; i++) nl_num += module_array[i]->m_n_in;

	//領域確保
	s_nl = (S_NLIST *)malloc(sizeof(S_NLIST) * nl_num);
	spi = (S_NLIST **)malloc(sizeof(S_NLIST *) * n_spi);
	spo = (S_NLIST **)malloc(sizeof(S_NLIST *) * n_spo);
	sdff = (S_NLIST **)malloc(sizeof(S_NLIST *) * n_sdff);
	srdff = (S_NLIST **)malloc(sizeof(S_NLIST *) * n_srdff);
	sdffs = (S_NLIST **)malloc(sizeof(S_NLIST *) * n_sdffs);
	srdffs = (S_NLIST **)malloc(sizeof(S_NLIST *) * n_srdffs);
	sassign = (S_NLIST **)malloc(sizeof(S_NLIST *) * n_sassign);
	...
	for(i=0; i<nl_num; i++) {
		s_nl[i].type		= -1;
		s_nl[i].n_in		= 0;
		s_nl[i].in			= (S_NLIST **)NULL;
		s_nl[i].n_out		= 0;
		s_nl[i].out			= (S_NLIST **)NULL;
		parser_array[0][i]	= 0;
		parser_array[1][i]	= 0;
	}
}
...
static void store_signal(void)
{
	...
	//入力
		for(j=0; j<module_array[i]->m_n_in; j++) {
			...
			while(ptr != NULL) {
				if(!strcmp(s_nl[ptr->nl_id].name, module_array[i]->m_in_name[j])) {
					//構文解析配列インクリメント
			...
			if(ptr == NULL) {
				...
				ptr->nl_id = n_snet;
				//ネットリスト挿入
				s_nl[n_snet].name = (char *)malloc(sizeof(char) * (strlen(module_array[i]->m_in_name[j]) + 1));
				strcpy(s_nl[n_snet].name, module_array[i]->m_in_name[j]);
				s_nl[n_snet].n = n_snet; //ネットリストID
				//構文解析配列代入
				parser_array[0][n_snet] = 1;
				//入力信号ネットリストID配列挿入
				module_array[i]->in_nl_id[j] = n_snet;
	...
	//出力
		...
		ptr = hash_t[hash_value];
		while(ptr != NULL) {
			if(!strcmp(s_nl[ptr->nl_id].name, module_array[i]->m_out_name)) {
				//構文解析配列インクリメント
		...
		if(ptr == NULL) {
			...
			ptr->nl_id = n_snet;
			//ネットリスト挿入
			s_nl[n_snet].name = (char *)malloc(sizeof(char) * (strlen(module_array[i]->m_out_name) + 1));
			strcpy(s_nl[n_snet].name, module_array[i]->m_out_name);
			s_nl[n_snet].n = n_snet; //ネットリストID
			//構文解析配列代入
			parser_array[1][n_snet] = 1;
			//出力信号ネットリストID挿入
            module_array[i]->out_nl_id = n_snet;
...
static void store_pi_po(FILE *rfp)
{
	...
	while(get_token(rfp, str, 0, 0) != EOF) {
		...
		else if(!strcmp(str, "assign")) {while((c = getc(rfp)) != ';'){} continue;}//read_assign(rfp, &assign_no); //空ループの明示として，;ではなく{}が推奨されています．
		else if(!strcmp(str, "wire")) {while((c = getc(rfp)) != ';'){} continue;} //空ループの明示として，;ではなく{}が推奨されています．
		else if(!strcmp(str, "endmodule")) {continue;}
		else {while((c = getc(rfp)) != ';'){} continue;} //空ループの明示として，;ではなく{}が推奨されています．
	}
}
...
static void hash_search_pi_po(char *word, int *no, int mode)
{
	...
	ptr = hash_t[hash_value];
	while(ptr != NULL) {
		if(!strcmp(s_nl[ptr->nl_id].name, word)) {
			//spi
			if(mode == 0) {
				//TYPE
				s_nl[ptr->nl_id].type = IN;
				//spi配列
				spi[(*no) ++] = &s_nl[ptr->nl_id];
			}
			//PO
			else if(mode == 1) {
				//PO配列
				spo[(*no) ++] = &s_nl[ptr->nl_id];
			}
			break;
		}
		ptr = ptr->next;
...
static void read_assign(FILE *rfp, int *assign_no)
{
	...
	if((c == '0') || (c == '1')) {
		s_nl[n_snet].n = n_snet;
		s_nl[n_snet].name = (char *)malloc(sizeof(char) * (strlen(str) + 1));
		strcpy(s_nl[n_snet].name, str);
		s_nl[n_snet].n_in = 0;
		s_nl[n_snet].n_out = 0;
		s_nl[n_snet].in = NULL;
		s_nl[n_snet].name_ins = NULL;
		s_nl[n_snet].name_port = NULL;
		s_nl[n_snet].out = NULL;
		if(c == '0') s_nl[n_snet].type = GND;
		else if(c == '1') s_nl[n_snet].type = ACC;
		while(c != ';') c = fgetc(rfp);	
...
static void analysis_asyntax_error(void)
{
	...
	for(i=0; i<n_snet; i++) {
		//spiバッティング
		if((parser_array[1][i]>=1) && (s_nl[i].type==IN))	error(4, s_nl[i].name);
		//内部信号バッティング
		else if(parser_array[1][i]>=2) error(5, s_nl[i].name);;
	}
}
...
static void alloc_nl_in_out(void)
{
	...
	//各信号 n_in の読み込み
	for(i=0; i<n_module; i++)
		s_nl[module_array[i]->out_nl_id].n_in = module_array[i]->m_n_in;

	//各信号 n_out の読み込み
	for(i=0; i<n_snet; i++) s_nl[i].n_out = parser_array[0][i];
	for(i=0; i<n_spo; i++) //POがFOUTしている場合
		if(parser_array[0][spo[i]->n] >= 1) spo[i]->n_out ++;

	//各信号 in,out 領域確保,初期化
	for(i=0; i<n_snet; i++) {
		if(s_nl[i].n_in != 0) {
			s_nl[i].in = (S_NLIST **)malloc(sizeof(S_NLIST *) * s_nl[i].n_in);
			for(j=0; j<s_nl[i].n_in; j++) s_nl[i].in[j] = NULL;
		}
		if(s_nl[i].n_out != 0) {
			s_nl[i].out = (S_NLIST **)malloc(sizeof(S_NLIST *) * s_nl[i].n_out);
			for(j=0; j<s_nl[i].n_out; j++) s_nl[i].out[j] = NULL;
		}
	}
}
...
static void connection_n_signal(void)
{
	...
	for(i=0; i<n_module; i++) {
		for(j=0; j<module_array[i]->m_n_in; j++) {
			//IN側がFOUTしていない場合
			if(s_nl[module_array[i]->in_nl_id[j]].n_out == 1) {
				connection_no_fout(i, j);
			}
			//IN側がFOUTしている場合
			else if(s_nl[module_array[i]->in_nl_id[j]].n_out >= 2) {
                connection_yes_fout(i, j);
				//POがFOUTしているかチェック
				for(k=0; k<n_spo; k++) {
					if(s_nl[module_array[i]->in_nl_id[j]].n == spo[k]->n) {
						//PO配列を書き換える
						spo[k] = &s_nl[n_snet];
...
static void connection_no_fout(int m_no, int in_no)
{
	...
	//IN側の出力信号
	s_nl[module_array[m_no]->in_nl_id[in_no]].out[0] = &s_nl[module_array[m_no]->out_nl_id];
	//OUT側の入力信号
	while(s_nl[module_array[m_no]->out_nl_id].in[no] != NULL) no ++;
	s_nl[module_array[m_no]->out_nl_id].in[no] = &s_nl[module_array[m_no]->in_nl_id[in_no]];
	//OUT側のTYPE
	s_nl[module_array[m_no]->out_nl_id].type = module_array[m_no]->m_type;
	//OUT側のインスタンス名
	s_nl[module_array[m_no]->out_nl_id].name_ins
		= (char *)malloc(sizeof(char) * (strlen(module_array[m_no]->m_name_ins) + 1));
	strcpy(s_nl[module_array[m_no]->out_nl_id].name_ins, module_array[m_no]->m_name_ins);
	//OUT側の端子名
	s_nl[module_array[m_no]->out_nl_id].name_port	= (char *)malloc(sizeof(char) * (1 + 1));
	if(s_nl[module_array[m_no]->out_nl_id].type == DFF || s_nl[module_array[m_no]->out_nl_id].type == RDFF ||
	   s_nl[module_array[m_no]->out_nl_id].type == DFFS ||s_nl[module_array[m_no]->out_nl_id].type == RDFFS) {
	    strcpy(s_nl[module_array[m_no]->out_nl_id].name_port, "Q");
	}
	else
		strcpy(s_nl[module_array[m_no]->out_nl_id].name_port, "Z");
}
...
static void connection_yes_fout(int m_no, int in_no)
{
	...
	//FOUT作成
	sprintf(tmp, "%s_%s_%c", s_nl[module_array[m_no]->in_nl_id[in_no]].name
		                  , s_nl[module_array[m_no]->out_nl_id].name
						  , s_pin_name_array[in_no]);
	s_nl[n_snet].name = (char *)malloc(sizeof(char) * (strlen(tmp) + 1));
	strcpy(s_nl[n_snet].name, tmp);
	s_nl[n_snet].n = n_snet;
	s_nl[n_snet].type = FOUT;
	s_nl[n_snet].n_in = 1;
	s_nl[n_snet].n_out = 1;
	s_nl[n_snet].in = (S_NLIST **)malloc(sizeof(S_NLIST *)); s_nl[n_snet].in[0] = NULL;
	s_nl[n_snet].out = (S_NLIST **)malloc(sizeof(S_NLIST *)); s_nl[n_snet].out[0] = NULL;
	//元のIN側の出力信号
	while(s_nl[module_array[m_no]->in_nl_id[in_no]].out[no] != NULL) no ++;
	s_nl[module_array[m_no]->in_nl_id[in_no]].out[no] = &s_nl[n_snet];
	no = 0;
	//OUT側の入力信号
	while(s_nl[module_array[m_no]->out_nl_id].in[no] != NULL) no ++;
	s_nl[module_array[m_no]->out_nl_id].in[no] = &s_nl[n_snet];
	no = 0;
	//OUT側のTYPE
	s_nl[module_array[m_no]->out_nl_id].type = module_array[m_no]->m_type;
	//FOUTの入力信号
    s_nl[n_snet].in[0] = &s_nl[module_array[m_no]->in_nl_id[in_no]];
	//FOUTの出力信号
	s_nl[n_snet].out[0] = &s_nl[module_array[m_no]->out_nl_id];
	//FOUTのインスタンス名
	s_nl[n_snet].name_ins
		= (char *)malloc(sizeof(char) * (strlen(module_array[m_no]->m_name_ins) + 1));
	strcpy(s_nl[n_snet].name_ins, module_array[m_no]->m_name_ins);
	//FOUTの端子名
	if(s_nl[module_array[m_no]->out_nl_id].type == DFF || s_nl[module_array[m_no]->out_nl_id].type == RDFF ||
	   s_nl[module_array[m_no]->out_nl_id].type == DFFS ||s_nl[module_array[m_no]->out_nl_id].type == RDFFS) {
		if(in_no == 0) {
			s_nl[n_snet].name_port	= (char *)malloc(sizeof(char) * (1 + 1));
			s_nl[n_snet].name_port[0] = 'D'; s_nl[n_snet].name_port[1] = '\0';
		}
		else if(in_no == 1) {
			if(s_nl[module_array[m_no]->out_nl_id].type == RDFF) {
				s_nl[n_snet].name_port	= (char *)malloc(sizeof(char) * (2 + 1));
				strcpy(s_nl[n_snet].name_port, "CD");
			}
			else if(s_nl[module_array[m_no]->out_nl_id].type == DFFS || s_nl[module_array[m_no]->out_nl_id].type == RDFFS) {
				s_nl[n_snet].name_port	= (char *)malloc(sizeof(char) * (2 + 1));
				strcpy(s_nl[n_snet].name_port, "TI");
			}
		}
		else if(in_no == 2) {
			s_nl[n_snet].name_port	= (char *)malloc(sizeof(char) * (2 + 1));
			strcpy(s_nl[n_snet].name_port, "TE");
		}
		else if(in_no == 3) {
			s_nl[n_snet].name_port	= (char *)malloc(sizeof(char) * (2 + 1));
			strcpy(s_nl[n_snet].name_port, "CD");
		...
		s_nl[n_snet].name_port	= (char *)malloc(sizeof(char) * (1 + 1));
		s_nl[n_snet].name_port[0] = s_pin_name_array[in_no];	s_nl[n_snet].name_port[1] = '\0';
	}
	//OUT側のインスタンス名
	s_nl[module_array[m_no]->out_nl_id].name_ins
		= (char *)malloc(sizeof(char) * (strlen(module_array[m_no]->m_name_ins) + 1));
...
static void connection_po_fout(int m_no, int in_no, int po_no)
{
	...
	//FOUT作成
	s_nl[n_snet].name = (char *)malloc(sizeof(char) * (strlen(s_nl[module_array[m_no]->in_nl_id[in_no]].name) + 1));
	strcpy(s_nl[n_snet].name, s_nl[module_array[m_no]->in_nl_id[in_no]].name);
	s_nl[n_snet].n = n_snet;
	s_nl[n_snet].type = FOUT;
	s_nl[n_snet].n_in = 1;
	s_nl[n_snet].in = (S_NLIST **)malloc(sizeof(S_NLIST *));
	s_nl[n_snet].in[0] = NULL;
	s_nl[n_snet].n_out = 0;
	//元のIN側の出力信号
	while(s_nl[module_array[m_no]->in_nl_id[in_no]].out[no] != NULL) no ++;
	s_nl[module_array[m_no]->in_nl_id[in_no]].out[no] = &s_nl[n_snet];
	free(s_nl[module_array[m_no]->in_nl_id[in_no]].name);
	sprintf(tmp, "%s_stem", s_nl[n_snet].name);
	s_nl[module_array[m_no]->in_nl_id[in_no]].name = (char *)malloc(sizeof(char) * (strlen(tmp) + 1));
	strcpy(s_nl[module_array[m_no]->in_nl_id[in_no]].name, tmp);
	//FOUTの入力信号
    s_nl[n_snet].in[0] = &s_nl[module_array[m_no]->in_nl_id[in_no]];
	//FOUTのインスタンス名,端子名
	s_nl[n_snet].name_ins
		= (char *)malloc(sizeof(char) * (strlen(s_nl[n_snet].name) + 1));
	strcpy(s_nl[n_snet].name_ins, s_nl[n_snet].name);
	s_nl[n_snet].name_port = NULL;
...
static void set_pi_ins(void)
{
	...
	for(i=0; i<n_spi; i++) {
		spi[i]->name_ins = (char *)malloc(sizeof(char) * (strlen(spi[i]->name) + 1));
		strcpy(spi[i]->name_ins, spi[i]->name);
		spi[i]->name_port = NULL;
	}
}
...
static void set_ff_array(void)
{
	...
	for(i=0; i<n_snet; i++) {
		switch (s_nl[i].type) {
			case DFF: sdff[dff_no ++] = &s_nl[i]; break;
			case DFFS: sdffs[dffs_no ++] = &s_nl[i]; break;
			case RDFF: srdff[rdff_no ++] = &s_nl[i]; break;
			case RDFFS:	srdffs[rdffs_no ++] = &s_nl[i]; break;
		}
	}
}
...
static void debug_nl(void)
{
	...
	printf("n_snet : %d\n", n_snet);
	printf("n_spi : %d ", n_spi);
	for(i=0; i<n_spi; i++) {
		printf("spi[%d]:%s ", i, spi[i]->name);
	}
    printf("\n");
	printf("n_po : %d ", n_po);
	for(i=0; i<n_po; i++) {
	...
	for(i=0; i<n_dff; i++) {
		printf("sdff[%d]:%s ", i, sdff[i]->name);
	}
	printf("\n");
	printf("n_rdff : %d ", n_rdff);
	for(i=0; i<n_rdff; i++) {
		printf("srdff[%d]:%s ", i, srdff[i]->name);
	}
	printf("\n");
	printf("n_dffs : %d ", n_dffs);
	for(i=0; i<n_dffs; i++) {
		printf("sdffs[%d]:%s ", i, sdffs[i]->name);
	}
	printf("\n");
	printf("n_rdffs : %d ", n_rdffs);
	for(i=0; i<n_rdffs; i++) {
		printf("srdffs[%d]:%s ", i, srdffs[i]->name);
	}
	printf("\n\n");

	for(i=0; i<n_snet; i++) {
		printf("s_nl[%d] name : %s\n", s_nl[i].n, s_nl[i].name);
		printf("type : %d ins_name : %s port_name : %s\n", s_nl[i].type, s_nl[i].name_ins, s_nl[i].name_port);
		printf("n_in : %d ", s_nl[i].n_in);
		for(j=0; j<s_nl[i].n_in; j++) {
			printf("in[%d]:%s ", j, s_nl[i].in[j]->name);
		}
		printf("\n");
		printf("n_out : %d ", s_nl[i].n_out);
		for(j=0; j<s_nl[i].n_out; j++) {
			printf("out[%d]:%s ", j, s_nl[i].out[j]->name);
		}
		printf("\n");
	}
	printf("\n");
