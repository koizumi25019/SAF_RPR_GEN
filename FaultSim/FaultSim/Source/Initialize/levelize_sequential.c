//------------------------------------------------------------------------
//File name : levelize_sequential.c
//Date : 2011/7/11
//Designer : H.Yamazaki
//Ver : 0.01
//------------------------------------------------------------------------
...
//	int		count;				//カウンタ 未使用変数のため削除
	...
	// メモリ確保
	//============================================================
	n_level = INT_alloc(n_snet);			//とりあえず全信号線数分確保
	level_stack = SPNL_alloc(n_snet);	//全信号線数分確保

	
	//============================================================
	// 初期化
	//============================================================
//	count			= 0;		//カウンタ初期化 未使用変数のため削除
	...
	for(i=0; i<n_spi; i++){
		...
		// PIをレベル0に設定
		spi[i]->level = 0;			//PIはゲート段数『0』
		spi[i]->flag  = LEVELED;		//レベル計算済み

		//【未レベライズ信号線探索】PI->out[j]をlevel_stackに積む
		for(j=0; j<spi[i]->n_out; j++){
			if(spi[i]->out[j]->flag != LEVELED){
				level_stack[n_level_stack] = spi[i]->out[j];		//level_stackに未レベライズ信号線を積む
				spi[i]->out[j]->flag = STACK;			//level_stackに積んでるよ
				n_level_stack++;						//level_stack格納数更新
			}
		}

		n_level[spi[i]->level]++;	//レベル【spi[i]->level】の信号線数を更新
	}
	...
	for(i=0; i<n_ppi; i++){
		...
		// PPIをレベル0に設定
		ppi[i]->level = 0;			//PPIはゲート段数『0』
		ppi[i]->flag  = LEVELED;	//レベル計算済み

		//【未レベライズ信号線探索】PPI->out[j]をlevel_stackに積む
		for(j=0; j<ppi[i]->n_out; j++){
			if(ppi[i]->out[j]->flag != LEVELED){
				level_stack[n_level_stack] = ppi[i]->out[j];	//level_stackに未レベライズ信号線を積む
				ppi[i]->out[j]->flag = STACK;					//level_stackに積んでるよ
				n_level_stack++;								//level_stack格納数更新
			}
		}

		n_level[ppi[i]->level]++;	//レベル【ppi[i]->level】の信号線数を更新
	}
	...
	//============================================================
	//メモリ確保
	nml_slev=(struct SLevelize*)calloc(max_level+1,sizeof(struct SLevelize));
	slev_temp=(struct SLevelize*)calloc(max_level+1,sizeof(struct SLevelize));
	...
	for(i=0; i<max_level+1; i++){
		...
		nml_slev[i].net		= NULL;
		nml_slev[i].n_net	= 0;
		slev_temp[i].net		= NULL;
		slev_temp[i].n_net	= 0;

		//メモリ確保(対象レベルの信号線数分)
		nml_slev[i].net = (S_NLIST **)calloc(n_level[i]+1,sizeof(S_NLIST*));
		slev_temp[i].net = (S_NLIST **)calloc(n_level[i]+1,sizeof(S_NLIST*));

		//レベライズ構造体の信号線ポインタ初期化
		for(j=0; j<n_level[i]+1; j++){
			nml_slev[i].net[j]	= NULL;
			slev_temp[i].net[j]	= NULL;
		}
	...
	for(i=0; i<n_snet; i++){
		//レベライズ構造体に格納
		nml_slev[s_nl[i].level].net[nml_slev[s_nl[i].level].n_net] = &s_nl[i];
		nml_slev[s_nl[i].level].n_net++;	//対象レベルに含まれる信号線数更新
	}
