...
{
	// 注: opb.total.vars はリセットしない（インクリメンタルSATのため、故障ごとに重複しない新規変数を採番する）
	for (int i = 0; i < 1; i++)
	{
...
)
{
	...
	cadd(solver, (int)target->netptr->varprop);
	cadd(solver, 0);
	...
	for (int i = 0; i < n_net; i++)
	{
		...
		if (nl[i].n_out > 0)
		{
			cadd(solver, -(int)nl[i].varprop);
			for (int j = 0; j < nl[i].n_out; j++)
			{
				cadd(solver, (int)nl[i].out[j]->varprop);
			}
			cadd(solver, 0);
		}
		...
		// ACT(X)=1 なら少なくとも一方が1
		cadd(solver, -(int)nl[i].varprop);
		cadd(solver, (int)nl[i].varsgc);
		cadd(solver, (int)nl[i].varsfc);
		cadd(solver, 0);
...
void CreateConsFC_AND(CCaDiCaL* solver, NLIST* netptr)
{
	...
	for (int i = 0; i < netptr->n_in; i++) {
		cadd(solver, -netptr->in[i]->varsfc);
	}
	cadd(solver, netptr->varsfc);
	cadd(solver, 0);
...
void CreateConsFC_BUF(CCaDiCaL* solver, NLIST* netptr)
{
	...
	cadd(solver, -in); cadd(solver, z);  cadd(solver, 0);
	cadd(solver, in);  cadd(solver, -z); cadd(solver, 0);
}
...
void CreateConsFC_XOR(CCaDiCaL* solver, NLIST* netptr)
{
	...
	cadd(solver, -a); cadd(solver, -b); cadd(solver, -z); cadd(solver, 0);
	cadd(solver, a);  cadd(solver, b);  cadd(solver, -z); cadd(solver, 0);
	cadd(solver, a);  cadd(solver, -b); cadd(solver, z);  cadd(solver, 0);
	cadd(solver, -a); cadd(solver, b);  cadd(solver, z);  cadd(solver, 0);
}
