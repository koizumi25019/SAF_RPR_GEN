...
void AllocMiterVars(void)
{
	...
	cnf.total.vars = cnf.constant.vars;   // 正常回路変数の直後から採番
	for (int i = 0; i < n_net; i++)
	{
		mt_act[i]  = ++cnf.total.vars;
		mt_sval[i] = ++cnf.total.vars;
		mt_vf[i]   = ++cnf.total.vars;
		mt_drv[i]  = (nl[i].type == IN || nl[i].type == DFF) ? 0 : ++cnf.total.vars;
	}
}

//*************************************************************************************************************
//	@name		FreeMiterVars
//	@function	ミター変数バッファを解放する
//*************************************************************************************************************
void FreeMiterVars(void)
{
	free(mt_act);  mt_act  = (int*)NULL;
	free(mt_sval); mt_sval = (int*)NULL;
	free(mt_vf);   mt_vf   = (int*)NULL;
	free(mt_drv);  mt_drv  = (int*)NULL;
}

//*************************************************************************************************************
//	@name		emit_gate
//	@function	z = gate(faulty値の入力) の節を投入（CreateConsGC_* と同じ論理）
//*************************************************************************************************************
...
void BuildFaultyMiter(CCaDiCaL* s)
{
	...
	for (int i = 0; i < n_net; i++)
	{
		...
		int f = mt_vf[i];
		int d = ++cnf.total.vars;          // d = g XOR f
